...
git submodule update --init stai-blockchain-gui
...
# Other than `stai-blockchain-gui/package/gui`, all other packages are no longer necessary after build.
# Since these unused packages make cache unnecessarily fat, unused packages should be removed.
Write-Output "Remove unused @stai-network packages to make cache slim"
...
# Remove `packages/gui/node_modules/@stai-network` because it causes an error on later `electron-packager` command
#Remove-Item "@stai-network" -Recurse -Force
