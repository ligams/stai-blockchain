...
LAST_EXIT_CODE=$?
ls -l dist/mac*/stai.app/Contents/Resources/app.asar
...
if [ "$(arch)" = "arm64" ]; then
  mv dist/"${DMG_NAME}" dist/stai-"${STAI_INSTALLER_VERSION}"-arm64.dmg
  DMG_NAME=stai-${STAI_INSTALLER_VERSION}-arm64.dmg
fi
...
# xcrun notarytool submit --wait --apple-id username --password password --team-id team-id Stai-0.1.X.dmg
