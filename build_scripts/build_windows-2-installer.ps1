...
Set-Location -Path "stai-blockchain-gui\packages\gui" -PassThru

Write-Output "   ---"
Write-Output "Increase the stack for stai command for (stai plots create) staipos limitations"
...
npx electron-builder build --win --x64 --config.productName="Stai" --dir --config ../../../build_scripts/electron-builder.json
...
npx electron-builder build --win --x64 --config.productName="Stai" --pd ".\dist\win-unpacked" --config ../../../build_scripts/electron-builder.json
Write-Output "   ---"

If ($env:HAS_SIGNING_SECRET) {
   Write-Output "   ---"
   Write-Output "Sign Final Installer App"
   signtool.exe sign /sha1 $env:SM_CODE_SIGNING_CERT_SHA1_HASH /tr http://timestamp.digicert.com /td SHA256 /fd SHA256 .\dist\StaiSetup-$packageVersion.exe
   Write-Output "   ---"
   Write-Output "Verify signature"
   Write-Output "   ---"
   signtool.exe verify /v /pa .\dist\StaiSetup-$packageVersion.exe
}   Else    {
   Write-Output "Skipping verify signatures - no authorization to install certificates"
}
...
Write-Output "   ---"
Copy-Item ".\dist\win-unpacked" -Destination "$env:GITHUB_WORKSPACE\stai-blockchain-gui\Stai-win32-x64" -Recurse
mkdir "$env:GITHUB_WORKSPACE\stai-blockchain-gui\release-builds\windows-installer" -ea 0
Copy-Item ".\dist\StaiSetup-$packageVersion.exe" -Destination "$env:GITHUB_WORKSPACE\stai-blockchain-gui\release-builds\windows-installer"
