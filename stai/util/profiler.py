...
# the output will be printed to your stai root path, e.g. ~/.stai/mainnet/profile/
# to analyze the profile, run:

#   python stai/utils/profiler.py ~/.stai/mainnet/profile | less -r

# this will print CPU usage of the stai full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python stai/utils/profiler.py ~/.stai/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int) -> None:
        ...
        output_file = "stai-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
