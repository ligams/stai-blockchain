...
from stai.cmds.init_funcs import stai_full_version_str
from stai.daemon.server import service_launch_lock_path
from stai.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from stai.server.api_protocol import ApiProtocol
from stai.server.stai_policy import set_stai_policy
from stai.server.outbound_message import NodeType
from stai.server.server import StaiServer
from stai.server.ssl_context import stai_ssl_ca_paths, private_ssl_ca_paths
from stai.server.upnp import UPnP
from stai.server.ws_connection import WSStaiConnection
from stai.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
from stai.util.network import resolve
...
class Service(Generic[_T_RpcServiceProtocol, _T_ApiProtocol, _T_RpcApiProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSStaiConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"stai-blockchain version: {stai_full_version_str()}")

        self.service_config = self.config[service_name]

        self._rpc_info = rpc_info
        private_ca_crt, private_ca_key = private_ssl_ca_paths(root_path, self.config)
        stai_ca_crt, stai_ca_key = stai_ssl_ca_paths(root_path, self.config)
        ...
        self._server = StaiServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def manage(self, *, start: bool = True) -> AsyncIterator[None]:
        ...
        async with contextlib.AsyncExitStack() as async_exit_stack:
            try:
                ...
                self._log.info("Waiting for StaiServer to be closed")
    ...
    async def setup_process_global_state(self, signal_handlers: SignalHandlers) -> None:
        ...
        proctitle_name = f"stai_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    with log_exceptions(log=log, message="fatal uncaught exception"):
        if connection_limit is not None:
            set_stai_policy(connection_limit)
