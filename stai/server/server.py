...
from stai.protocols.protocol_state_machine import message_requires_reply
from stai.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
from stai.protocols.shared_protocol import protocol_version
from stai.server.api_protocol import ApiProtocol
from stai.server.introducer_peers import IntroducerPeers
from stai.server.outbound_message import Message, NodeType
from stai.server.ssl_context import private_ssl_paths, public_ssl_paths
from stai.server.ws_connection import ConnectionCallback, WSStaiConnection
from stai.types.blockchain_format.sized_bytes import bytes32
from stai.types.peer_info import PeerInfo
from stai.util.errors import Err, ProtocolError
from stai.util.ints import uint16
from stai.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
from stai.util.ssl_check import verify_ssl_certs_and_keys
...
class StaiServer:
    ...
    all_connections: Dict[bytes32, WSStaiConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        stai_ca_crt_key: Tuple[Path, Path],
        name: str = __name__,
    ) -> StaiServer:
        ...
        stai_ca_crt_path, stai_ca_key_path = stai_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=stai_ca_crt_path,
                ca_key=stai_ca_key_path,
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSStaiConnection] = []
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("StaiServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSStaiConnection] = None
        try:
            connection = WSStaiConnection.create(
    ...
    async def connection_added(
        self, connection: WSStaiConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    async def connection_closed(
        self, connection: WSStaiConnection, ban_time: int, closed_connection: bool = False
    ) -> None:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use stai's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.stai.net/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSStaiConnection, trusted_peers: Dict[str, Any]) -> bool:
