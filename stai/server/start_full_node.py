...
from stai.server.start_service import RpcInfo, Service, async_run
from stai.types.aliases import FullNodeService
from stai.util.stai_logging import initialize_service_logging
from stai.util.config import load_config, load_config_cli
...
from stai.util.task_timing import maybe_manage_task_instrumentation
...
def main() -> int:
    ...
    with maybe_manage_task_instrumentation(enable=os.environ.get("STAI_INSTRUMENT_NODE") is not None):
        ...
        if not service_config.get("use_stai_loop_policy", True):
            target_peer_count = None
