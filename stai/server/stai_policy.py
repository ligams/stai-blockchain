...
class StaiSelectorEventLoop(asyncio.SelectorEventLoop):
    ...
    async def create_server(self, *args, **kwargs) -> PausableServer:  # type: ignore[no-untyped-def]
        return await _stai_create_server(super(), *args, **kwargs)


if sys.platform == "win32":

    class StaiProactor(IocpProactor):
        ...
        async def _stai_accept_loop(self, listener: socket.socket) -> Tuple[socket.socket, Tuple[object, ...]]:
            while True:
                ...
                try:
                    return await self._stai_accept(listener)
        ...
        def _stai_accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            ...
            async def accept_coro(self: StaiProactor, future: asyncio.Future[object], conn: socket.socket) -> None:
        ...
        def accept(self, listener: socket.socket) -> asyncio.Future[Tuple[socket.socket, Tuple[object, ...]]]:
            coro = self._stai_accept_loop(listener)
            return asyncio.ensure_future(coro)

    class StaiProactorEventLoop(ProactorEventLoop):
...
class StaiPolicy(asyncio.DefaultEventLoopPolicy):
    def new_event_loop(self) -> asyncio.AbstractEventLoop:
        # overriding https://github.com/python/cpython/blob/v3.11.0/Lib/asyncio/events.py#L689-L695
        if sys.platform == "win32":
            loop_factory = StaiProactorEventLoop
        else:
            loop_factory = StaiSelectorEventLoop

        return loop_factory()


def set_stai_policy(connection_limit: int) -> None:
    ...
    asyncio.set_event_loop_policy(StaiPolicy())
