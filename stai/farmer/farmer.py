...
from stai.plot_sync.delta import Delta
from stai.plot_sync.receiver import Receiver
from stai.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from stai.server.server import StaiServer, ssl_context_for_root
...
from stai.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from stai.util.errors import KeychainProxyConnectionFailure
...
from stai.util.logging import TimedDuplicateFilter
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'stai keys generate' or open the UI."
        try:
        ...
        if "stai_target_address" not in self.config:
            self.config = config["farmer"]
        if "stai_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "stai_target_address" not in self.config or "stai_target_address" not in self.pool_config:
            log.debug("stai_target_address missing in the config")
            return False
