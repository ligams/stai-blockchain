...
from stai.consensus.pot_iterations import calculate_iterations_quality, calculate_sp_interval_iters
from stai.farmer.farmer import Farmer, increment_pool_stats, strip_old_entries
from stai.harvester.harvester_api import HarvesterAPI
from stai.protocols import farmer_protocol, harvester_protocol
from stai.protocols.farmer_protocol import DeclareProofOfSpace, SignedValues
from stai.protocols.harvester_protocol import (
    PlotSyncDone,
    PlotSyncPathList,
    PlotSyncPlotList,
    PlotSyncStart,
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: WSStaiConnection
    ) -> None:
        """
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Stai Blockchain v.{__version__}"},
                        ) as resp:
                            if not resp.ok:
    ...
    @api_request(peer_required=True)
    async def farming_info(self, request: farmer_protocol.FarmingInfo, peer: WSStaiConnection) -> None:
        self.farmer.state_changed(
            "new_farming_info",
            {
                "farming_info": {
    ...
    @api_request(peer_required=True)
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: WSStaiConnection) -> None:
        self.farmer.log.warning(f"Respond plots came too late from: {peer.get_peer_logging()}")

    @api_request(peer_required=True)
    async def plot_sync_start(self, message: PlotSyncStart, peer: WSStaiConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].sync_started(message)

    @api_request(peer_required=True)
    async def plot_sync_loaded(self, message: PlotSyncPlotList, peer: WSStaiConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_loaded(message)

    @api_request(peer_required=True)
    async def plot_sync_removed(self, message: PlotSyncPathList, peer: WSStaiConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_removed(message)

    @api_request(peer_required=True)
    async def plot_sync_invalid(self, message: PlotSyncPathList, peer: WSStaiConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_invalid(message)

    @api_request(peer_required=True)
    async def plot_sync_keys_missing(self, message: PlotSyncPathList, peer: WSStaiConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_keys_missing(message)

    @api_request(peer_required=True)
    async def plot_sync_duplicates(self, message: PlotSyncPathList, peer: WSStaiConnection) -> None:
        await self.farmer.plot_sync_receivers[peer.peer_node_id].process_duplicates(message)

    @api_request(peer_required=True)
    async def plot_sync_done(self, message: PlotSyncDone, peer: WSStaiConnection) -> None:
