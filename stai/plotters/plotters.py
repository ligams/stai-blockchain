...
from stai.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from stai.plotters.chiapos import get_chiapos_install_info, plot_stai
from stai.plotters.madmax import get_madmax_install_info, plot_madmax
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in STAI_ROOT.
    stai_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within STAI_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
            print(f"Cannot create plotters root path {root_path} {type(e)} {e}.")

    plotters = argparse.ArgumentParser("stai plotters", description="Available options.")
    subparsers = plotters.add_subparsers(help="Available options", dest="plotter")

    build_parser(subparsers, root_path, stai_plotter_options, "chiapos", "Create a plot with the default stai plotter")
    ...
    deprecation_warning = (
        "[DEPRECATED] 'stai plotters install' is no longer available. Use install-plotter.sh/ps1 instead."
    )
    subparsers.add_parser("install", help=deprecation_warning, add_help=False)

    deprecation_warning_bb2 = "[DEPRECATED] 'stai plotters bladebit2' was integrated to 'stai plotters bladebit'"
    ...
    if args.plotter is None:
        ...
        plot_stai(args, stai_root_path)
    elif args.plotter == "madmax":
        plot_madmax(args, stai_root_path, root_path)
    elif args.plotter == "bladebit":
        plot_bladebit(args, stai_root_path, root_path)
    elif args.plotter == "version":
        show_plotters_version(stai_root_path)
