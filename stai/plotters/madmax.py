...
from stai.plotters.plotters_util import get_venv_bin, reset_loop_policy_for_windows, run_command, run_plotter
from stai.plotting.create_plots import resolve_plot_keys
...
def get_madmax_exec_venv_path(ksize: int = 32) -> Optional[Path]:
    ...
    if not venv_bin_path:
        return None
    madmax_exec = "stai_plot"
    if ksize > 32:
        madmax_exec += "_k34"  # Use the stai_plot_k34 executable for k-sizes > 32
...
def plot_madmax(args, stai_root_path: Path, plotters_root_path: Path):
    ...
    try:
        asyncio.run(run_plotter(stai_root_path, args.plotter, call_args, progress))
    except Exception as e:
