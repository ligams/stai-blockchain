...
from stai.consensus.block_body_validation import ForkInfo
from stai.consensus.block_creation import unfinished_block_to_full_block
from stai.consensus.block_record import BlockRecord
from stai.consensus.blockchain import AddBlockResult, Blockchain, BlockchainMutexPriority, StateChangeSummary
from stai.consensus.blockchain_interface import BlockchainInterface
...
from stai.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from stai.consensus.multiprocess_validation import PreValidationResult
from stai.consensus.pot_iterations import calculate_sp_iters
...
from stai.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
from stai.full_node.hint_management import get_hints_and_subscription_coin_ids
from stai.full_node.hint_store import HintStore
from stai.full_node.mempool_manager import MempoolManager
from stai.full_node.signage_point import SignagePoint
from stai.full_node.subscriptions import PeerSubscriptions
from stai.full_node.sync_store import Peak, SyncStore
from stai.full_node.tx_processing_queue import TransactionQueue
from stai.full_node.weight_proof import WeightProofHandler
from stai.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from stai.protocols.full_node_protocol import RequestBlocks, RespondBlock, RespondBlocks, RespondSignagePoint
from stai.protocols.protocol_message_types import ProtocolMessageTypes
from stai.protocols.wallet_protocol import CoinState, CoinStateUpdate
from stai.rpc.rpc_server import StateChangedProtocol
from stai.server.node_discovery import FullNodePeers
from stai.server.outbound_message import Message, NodeType, make_msg
...
from stai.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof, validate_vdf
from stai.types.coin_record import CoinRecord
from stai.types.end_of_slot_bundle import EndOfSubSlotBundle
...
from stai.types.transaction_queue_entry import TransactionQueueEntry
from stai.types.unfinished_block import UnfinishedBlock
from stai.types.weight_proof import WeightProof
from stai.util import cached_bls
from stai.util.bech32m import encode_puzzle_hash
from stai.util.check_fork_next_block import check_fork_next_block
from stai.util.condition_tools import pkm_pairs
from stai.util.config import PEER_DB_PATH_KEY_DEPRECATED, process_config_start_method
from stai.util.db_synchronous import db_synchronous_on
from stai.util.db_version import lookup_db_version, set_db_version_async
from stai.util.db_wrapper import DBWrapper2, manage_connection
from stai.util.errors import ConsensusError, Err, TimestampError, ValidationError
from stai.util.ints import uint8, uint32, uint64, uint128
from stai.util.limited_semaphore import LimitedSemaphore
from stai.util.log_exceptions import log_exceptions
from stai.util.path import path_from_root
from stai.util.profiler import mem_profile_task, profile_task
from stai.util.safe_cancel_task import cancel_task_safe
...
class FullNode:
    if TYPE_CHECKING:
        from stai.rpc.rpc_server import RpcServiceProtocol
    ...
    _server: Optional[StaiServer] = None
                    ...
                    async with conn.execute(
                        ...
                        if len(list(await cur.fetchall())) == 0:
                            try:
                                ...
                                # it could be a database created with "stai init", which is
    ...
    def set_server(self, server: StaiServer) -> None:
        self._server = server
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.stai.net")
        try:
    ...
    async def short_sync_batch(self, peer: WSStaiConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: WSStaiConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ) -> bool:
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSStaiConnection) -> None:
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[WSStaiConnection] = None
    ) -> None:
        """
    ...
    async def on_connect(self, connection: WSStaiConnection) -> None:
        """
    ...
    async def on_disconnect(self, connection: WSStaiConnection) -> None:
    ...
    async def request_validate_wp(
        ...
        weight_proof_peer: WSStaiConnection = random.choice(peers_with_peak)
        self.log.info(
    ...
    async def sync_from_fork_point(
        ...
        peers_with_peak: List[WSStaiConnection] = self.get_peers_with_peak(peak_hash)
        ...
        async def fetch_block_batches(
            batch_queue: asyncio.Queue[Optional[Tuple[WSStaiConnection, List[FullBlock]]]]
        ) -> None:
            start_height, end_height = 0, 0
            new_peers_with_peak: List[WSStaiConnection] = peers_with_peak[:]
            try:
        ...
        async def validate_block_batches(
            inner_batch_queue: asyncio.Queue[Optional[Tuple[WSStaiConnection, List[FullBlock]]]]
        ) -> None:
            fork_info: Optional[ForkInfo] = None

            while True:
                res: Optional[Tuple[WSStaiConnection, List[FullBlock]]] = await inner_batch_queue.get()
                if res is None:
        ...
        batch_queue_input: asyncio.Queue[Optional[Tuple[WSStaiConnection, List[FullBlock]]]] = asyncio.Queue(
            maxsize=buffer_size
        )
    ...
    def get_peers_with_peak(self, peak_hash: bytes32) -> List[WSStaiConnection]:
    ...
    async def peak_post_processing(
        ...
        peer: Optional[WSStaiConnection],
    ...
    async def add_block(
        self,
        block: FullBlock,
        peer: Optional[WSStaiConnection] = None,
    ...
    async def add_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'stai')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[WSStaiConnection] = None
    ) -> Optional[Message]:
    ...
    async def add_end_of_sub_slot(
        self, end_of_slot_bundle: EndOfSubSlotBundle, peer: WSStaiConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: WSStaiConnection) -> None:
        peak = self.blockchain.get_peak()
        if peak is None or peak.height - request.height < 5:
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: WSStaiConnection) -> None:
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def add_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: WSStaiConnection) -> None:
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: WSStaiConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
    block_response: Optional[Any] = await peer.call_api(
