...
from stai.cmds.wallet_funcs import print_balance, wallet_coin_unit
from stai.pools.pool_config import PoolWalletConfig, load_pool_config, update_pool_config
from stai.pools.pool_wallet_info import PoolSingletonState, PoolWalletInfo
...
async def create(
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, _):
        fee_mojos = uint64(int(fee * units["stai"]))
        ...
        if user_input.lower() == "y" or user_input.lower() == "yes":
            try:
                ...
                print(
                    f"Error creating plot NFT: {e}\n    Please start both farmer and wallet with:  stai start -r farmer"
                )
            return
        print("Aborting.")
