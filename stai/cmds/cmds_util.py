...
from stai.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate
...
from stai.types.mempool_submission_status import MempoolSubmissionStatus
from stai.util.config import load_config
from stai.util.default_root import DEFAULT_ROOT_PATH
from stai.util.errors import CliRpcConnectionError
from stai.util.ints import uint16, uint64
from stai.util.keychain import KeyData
from stai.util.streamable import Streamable, streamable
from stai.wallet.transaction_record import TransactionRecord
from stai.wallet.util.tx_config import CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig, TXConfigLoader
...
def transaction_status_msg(fingerprint: int, tx_id: bytes32) -> str:
    return f"Run 'stai wallet get_transaction -f {fingerprint} -tx 0x{tx_id}' to get status"
    ...
    try:
        if fingerprint is not None:
            ...
            if len(fingerprints) == 0:
                raise CliRpcConnectionError("No keys loaded. Run 'stai keys generate' or import a key")
...
def coin_selection_args(func: Callable[..., None]) -> Callable[..., None]:
    return click.option(
        ...
        "--min-coin-amount",
        "--min-amount",
        help="Ignore coins worth less then this much STAI or CAT units",
        type=str,
        required=False,
        default=None,
    )(
        click.option(
            ...
            "--max-coin-amount",
            "--max-amount",
            help="Ignore coins worth more then this much STAI or CAT units",
            type=str,
            required=False,
            default=None,
        )(
            click.option(
                ...
                click.option(
                    ...
                    "amounts_to_exclude",
                    multiple=True,
                    help="Exclude any coins with this STAI or CAT amount from being included.",
                )(func)
            )
        )
    )
