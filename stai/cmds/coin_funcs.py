...
from stai.cmds.cmds_util import CMDCoinSelectionConfigLoader, CMDTXConfigLoader, cli_confirm, get_wallet_client
from stai.cmds.units import units
from stai.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type, print_balance
...
from stai.util.ints import uint64, uint128
...
async def async_combine(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        final_fee = uint64(int(fee * units["stai"]))
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return
        is_stai: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly combining Stai
        ...
        if final_target_coin_amount != 0:  # if we have a set target, just use standard coin selection.
            removals: List[Coin] = await wallet_client.select_coins(
                amount=(final_target_coin_amount + final_fee) if is_stai else final_target_coin_amount,
                wallet_id=wallet_id,
        ...
        if is_stai and total_amount - final_fee <= 0:
        ...
        additions = [{"amount": (total_amount - final_fee) if is_stai else total_amount, "puzzle_hash": target_ph}]
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fingerprint) as (wallet_client, fingerprint, config):
        ...
        if not await wallet_client.get_synced():
            print("Wallet not synced. Please wait.")
            return
        is_stai: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly spitting Stai
        ...
        print(f"Transaction sent: {tx_id}")
        print(f"To get status, use command: stai wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
        dust_threshold = config.get("sati_spam_amount", 1000000)  # min amount per coin in mojo
