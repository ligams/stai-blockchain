...
from stai.util.beta_metrics import metrics_log_interval_max, metrics_log_interval_min
from stai.util.stai_logging import get_beta_logging_config
from stai.util.errors import InvalidPathError
from stai.util.misc import format_bytes, prompt_yes_no, validate_directory_writable


def default_beta_root_path() -> Path:
    return Path(os.path.expanduser(os.getenv("STAI_BETA_ROOT", "~/stai-beta-test"))).resolve()


def warn_if_beta_enabled(config: Dict[str, Any]) -> None:
    if config.get("beta", {}).get("enabled", False):
        print("\nWARNING: beta test mode is enabled. Run `stai beta disable` if this is unintentional.\n")
...
def prepare_stai_blockchain_log(path: Path) -> None:
    # TODO: Do stuff we want to do with the logs before submission. Maybe even just fully parse them and
    #  create some final result files and zip them instead of just the logs.
    print(f"  - {path.name}")
