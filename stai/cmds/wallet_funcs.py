...
from stai.cmds.cmds_util import (
    CMDTXConfigLoader,
    cli_confirm,
    get_wallet_client,
...
from stai.cmds.peer_funcs import print_connections
from stai.cmds.units import units
from stai.rpc.wallet_rpc_client import WalletRpcClient
from stai.types.blockchain_format.sized_bytes import bytes32
from stai.util.bech32m import bech32_decode, decode_puzzle_hash, encode_puzzle_hash
from stai.util.config import selected_network_address_prefix
from stai.util.ints import uint16, uint32, uint64
from stai.wallet.nft_wallet.nft_info import NFTInfo
...
from stai.wallet.transaction_sorting import SortKey
from stai.wallet.util.address_type import AddressType, ensure_valid_address
from stai.wallet.util.puzzle_decorator_type import PuzzleDecoratorType
...
from stai.wallet.wallet_coin_store import GetCoinRecords
...
def print_transaction(
    ...
    if verbose:
        ...
        stai_amount = Decimal(int(tx.amount)) / mojo_per_unit
        ...
        print(f"Amount {description}: {stai_amount} {name}")
...
async def send(
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        final_fee: uint64 = uint64(int(fee * units["stai"]))  # fees are always in STAI mojos
        ...
        print(f"To get status, use command: stai wallet get_transaction -f {fingerprint} -tx 0x{tx_id}")
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if offers == [] or requests == []:
            ...
            for item in [*offers, *requests]:
                            ...
                            if item in requests:
                                ...
                                if info.supports_did:
                                    ...
                                    royalty_asset_dict[name] = (
                                        encode_puzzle_hash(info.royalty_puzzle_hash, AddressType.STAI.hrp(config)),
                ...
                if fee > 0:
                    print()
                    print(f"Including Fees: {Decimal(fee) / units['stai']} STAI, {fee} mojos")
                ...
                if royalty_asset_dict != {}:
                    ...
                    for nft_id, summaries in royalty_summary.items():
                        ...
                        for summary in summaries:
                            divisor = units["stai"] if summary["asset"] == "STAI" else units["cat"]
                    ...
                    for asset, requested_amount in total_amounts_requested.items():
                        divisor = units["stai"] if asset == "STAI" else units["cat"]
                ...
                offer, trade_record = await wallet_client.create_offer_for_ids(
                    ...
                    tx_config=CMDTXConfigLoader(
                        reuse_puzhash=reuse_puzhash,
                    ).to_tx_config(units["stai"], config, fingerprint),
                )
                if offer is not None:
                    ...
                    print(
                        f"Use stai wallet get_offers --id " f"{trade_record.trade_id} -f {fingerprint} to view status"
                    )
                else:
...
async def print_offer_summary(
    cat_name_resolver: CATNameResolver, sum_dict: Dict[str, int], has_fee: bool = False, network_stai: str = "STAI"
) -> None:
    for asset_id, amount in sum_dict.items():
        description: str = ""
        unit: int = units["stai"]
        wid: str = "1" if asset_id == "stai" else ""
        mojo_amount: int = int(Decimal(amount))
        name: str = network_stai
        if asset_id != "stai":
            name = asset_id
            if asset_id == "unknown":
                name = "Unknown"
...
async def print_trade_record(record: TradeRecord, wallet_client: WalletRpcClient, summaries: bool = False) -> None:
    ...
    if summaries:
        ...
        print(f"Included Fees: {fees / units['stai']} XCH, {fees} mojos")
    print("---------------")
...
async def take_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        offered, requested, _, _ = offer.summary()
        cat_name_resolver = wallet_client.cat_asset_id_to_name
        network_stai = AddressType.XCH.hrp(config).upper()
        print("Summary:")
        print("  OFFERED:")
        await print_offer_summary(cat_name_resolver, offered, network_stai=network_stai)
        print("  REQUESTED:")
        await print_offer_summary(cat_name_resolver, requested, network_stai=network_stai)
        ...
        if royalty_asset_dict != {}:
            ...
            for fungible_asset_id in fungible_assets_from_offer(offer):
                fungible_asset_id_str = fungible_asset_id.hex() if fungible_asset_id is not None else "stai"
                if fungible_asset_id_str in requested:
                    nft_royalty_currency: str = "Unknown CAT"
                    if fungible_asset_id is None:
                        nft_royalty_currency = network_stai
                    else:
            ...
            if fungible_asset_dict != {}:
                ...
                for nft_id, summaries in royalty_summary.items():
                    print(f"  - For {nft_id}:")
                    for summary in summaries:
                        divisor = units["stai"] if summary["asset"] == network_stai else units["cat"]
                        converted_amount = Decimal(summary["amount"]) / divisor
                        total_amounts_requested.setdefault(summary["asset"], fungible_asset_dict[summary["asset"]])
                        total_amounts_requested[summary["asset"]] += summary["amount"]
                        print(
                ...
                for asset, amount in total_amounts_requested.items():
                    divisor = units["stai"] if asset == network_stai else units["cat"]
                    converted_amount = Decimal(amount) / divisor
                    print(f"  - {converted_amount} {asset} ({amount} mojos)")

        print(f"Included Fees: {Decimal(offer.fees()) / units['stai']} {network_stai}, {offer.fees()} mojos")
        ...
        if not examine_only:
            ...
            trade_record = await wallet_client.take_offer(
                offer,
                fee=fee,
                tx_config=CMDTXConfigLoader().to_tx_config(units["stai"], config, fingerprint),
            )
            print(f"Accepted offer with ID {trade_record.trade_id}")
            print(f"Use stai wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view its status")
...
async def cancel_offer(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        await wallet_client.cancel_offer(
            offer_id, CMDTXConfigLoader().to_tx_config(units["stai"], config, fingerprint), secure=secure, fee=fee
        )
        print(f"Cancelled offer with ID {trade_record.trade_id}")
        if secure:
            print(f"Use stai wallet get_offers --id {trade_record.trade_id} -f {fingerprint} to view cancel status")
...
def wallet_coin_unit(typ: WalletType, address_prefix: str) -> Tuple[str, int]:  # pragma: no cover
    ...
    if typ in [WalletType.STANDARD_WALLET, WalletType.POOLING_WALLET, WalletType.MULTI_SIG]:
        return address_prefix, units["stai"]
    return "", units["mojo"]
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        try:
            response = await wallet_client.did_message_spend(
                ...
                coin_announcements,
                CMDTXConfigLoader().to_tx_config(units["stai"], config, fingerprint),
            )
...
async def send_notification(
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        amount: uint64 = uint64(d_amount * units["stai"])
        message: bytes = bytes(message_hex, "utf8")
        fee: uint64 = uint64(d_fee * units["stai"])

        tx = await wallet_client.send_notification(address, message, amount, fee)

        print("Notification sent successfully.")
        print(f"To get status, use command: stai wallet get_transaction -f {fingerprint} -tx 0x{tx.name}")
    ...
    *, wallet_rpc_port: Optional[int], fp: Optional[int], fee: Decimal, tx_ids_str: str, force: bool = False
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        if fee < 0:
            ...
            return
        response = await wallet_client.spend_clawback_coins(tx_ids, int(fee * units["stai"]), force)
        print(str(response))
    ...
    *,
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        txs = await wallet_client.vc_spend(
            ...
            fee=uint64(int(d_fee * units["stai"])),
            tx_config=CMDTXConfigLoader(
                reuse_puzhash=reuse_puzhash,
            ).to_tx_config(units["stai"], config, fingerprint),
        )
