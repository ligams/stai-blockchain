...
def validate_fee(ctx: click.Context, param: click.Parameter, value: str) -> str:
    try:
        ...
        raise click.BadParameter("Fee must be decimal dotted value in STAI (e.g. 0.00005)")
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in STAI. Fee is used TWICE: once to create the singleton, once for init.",
    type=str,
    default="0",
    show_default=True,
    required=True,
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in STAI. Fee is used TWICE: once to leave pool, once to join.",
    type=str,
    default="0",
    show_default=True,
    required=True,
