...
from stai.cmds.passphrase_funcs import obtain_current_passphrase
...
from stai.util.errors import KeychainException
from stai.util.file_keyring import MAX_LABEL_LENGTH
from stai.util.ints import uint32
from stai.util.keychain import Keychain, KeyData, bytes_to_mnemonic, generate_mnemonic, mnemonic_to_seed
...
def generate_and_print() -> str:
    ...
    print(mnemonic)
    print("Note that this key has not been added to the keychain. Run stai keys add")
    return mnemonic
...
def show_all_key_labels() -> None:
    ...
    if len(keys) == 0:
        sys.exit("No keys are present in the keychain. Generate them with 'stai keys generate'")
...
def derive_sk_from_hd_path(master_sk: PrivateKey, hd_path_root: str) -> Tuple[PrivateKey, str]:
    ...
    from stai.wallet.derive_keys import _derive_path, _derive_path_unhardened
