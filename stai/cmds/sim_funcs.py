...
from stai.cmds.cmds_util import get_any_service_client
from stai.cmds.start_funcs import async_start
from stai.consensus.coinbase import create_puzzlehash_for_pk
from stai.server.outbound_message import NodeType
from stai.simulator.simulator_full_node_rpc_client import SimulatorFullNodeRpcClient
...
from stai.util.config import load_config, save_config, set_peer_info
...
def create_stai_directory(
    ...
    """
    This function creates a new stai directory and returns a heavily modified config,
    suitable for use in the simulator.
    """
    from stai.cmds.init_funcs import stai_init

    if not stai_root.is_dir() or not Path(stai_root / "config" / "config.yaml").exists():
        # create stai directories & load config
        stai_init(stai_root, testnet=True, fix_ssl_permissions=True)
        config: Dict[str, Any] = load_config(stai_root, "config.yaml")
...
def display_key_info(fingerprint: int, prefix: str) -> None:
    """
    Display key info for a given fingerprint, similar to the output of `stai keys show`.
    """
...
def generate_and_return_fingerprint(mnemonic: Optional[str] = None) -> int:
    ...
    """
    from stai.util.keychain import generate_mnemonic
...
async def generate_plots(config: Dict[str, Any], root_path: Path, fingerprint: int, bitfield: bool) -> None:
    ...
    from stai.simulator.start_simulator import PLOT_SIZE, PLOTS

    farming_puzzle_hash = decode_puzzle_hash(config["simulator"]["farming_address"])
    os.environ["STAI_ROOT"] = str(root_path)  # change env variable, to make it match what the daemon would set it to
...
async def async_config_wizard(
    ...
    if fingerprint is None:
        ...
        return
    # create stai directory & get config.
    print("Creating stai directory & config...")
    config = create_stai_directory(root_path, fingerprint, farming_address, plot_directory, auto_farm, docker_mode)
    ...
    sys.argv[0] = str(Path(sys.executable).parent / "stai")  # fix path for tests
    ...
    while True:
        try:
            ...
            break
    print(f"\nMake sure your STAI_ROOT Environment Variable is set to: {root_path}")
...
async def print_status(
    ...
    """
    from stai.cmds.show_funcs import print_blockchain_state
    ...
    async with get_any_service_client(SimulatorFullNodeRpcClient, rpc_port, root_path) as (node_client, config):
        ...
        # chain status ( basically stai show -s)
        ...
        print(
            ...
            f"with a balance of: "
            f"{sum(coin_records.coin.amount for coin_records in farming_coin_records) / units['stai']} TSTAI."
        )
        if show_addresses:
            print("All Addresses: ")
