...
from stai.util.service_groups import all_groups, services_for_groups


async def async_stop(root_path: Path, config: Dict[str, Any], group: tuple[str, ...], stop_daemon: bool) -> int:
    from stai.daemon.client import connect_to_daemon_and_validate

    daemon = await connect_to_daemon_and_validate(root_path, config)
    if daemon is None:
        print("Couldn't connect to stai daemon")
        return 1
...
def stop_cmd(ctx: click.Context, daemon: bool, group: tuple[str, ...]) -> None:
    from stai.cmds.beta_funcs import warn_if_beta_enabled
