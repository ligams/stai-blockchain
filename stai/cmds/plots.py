...
from stai.plotting.util import add_plot_directory, validate_plot_size

log = logging.getLogger(__name__)


def show_plots(root_path: Path) -> None:
    from stai.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'stai plots add -d [dir]' and remove with"
        + " 'stai plots remove -d [dir]'"
        + " Scan and check plots with 'stai plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context) -> None:
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'stai init'")
...
def create_cmd(
    ...
    from stai.plotting.create_plots import create_plots, resolve_plot_keys
    from stai.plotting.util import Params
...
def check_cmd(
    ...
    from stai.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str) -> None:
    from stai.plotting.util import remove_plot_directory
