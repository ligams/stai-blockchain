...
from stai.cmds.beta_funcs import (
    default_beta_root_path,
    prepare_stai_blockchain_log,
    prepare_logs,
...
from stai.util.beta_metrics import metrics_log_interval_default
from stai.util.config import lock_and_load_config, save_config


def print_restart_warning() -> None:
    print("\nRestart the daemon and any running stai services for changes to take effect.")
...
def configure(ctx: click.Context, path: Optional[str], interval: Optional[int]) -> None:
    ...
    with lock_and_load_config(root_path, "config.yaml") as config:
        if "beta" not in config:
            raise click.ClickException("beta test mode is not enabled, enable it first with `stai beta enable`")
...
def prepare_submission_cmd(ctx: click.Context) -> None:
    with lock_and_load_config(ctx.obj["root_path"], "config.yaml") as config:
        ...
        if beta_root_path is None:
            raise click.ClickException("beta test mode not enabled. Run `stai beta enable` first.")
    ...
    stai_blockchain_path = Path(prepare_result / "stai-blockchain")
    stai_logs = prepare_logs(plotting_path, prepare_stai_blockchain_log)
    plotting_logs = prepare_logs(stai_blockchain_path, prepare_plotting_log)
    ...
    with zipfile.ZipFile(submission_file_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
        files_added = add_files(stai_logs) + add_files(plotting_logs)

    if files_added == 0:
        submission_file_path.unlink()
        message = f"No logs files found in {str(plotting_path)!r} and {str(stai_blockchain_path)!r}."
