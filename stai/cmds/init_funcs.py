...
from stai.cmds.configure import configure
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'stai keys generate'")
        return None

    with lock_and_load_config(new_root, "config.yaml") as config:
        pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
        all_targets = []
        stop_searching_for_farmer = "stai_target_address" not in config["farmer"]
        stop_searching_for_pool = "stai_target_address" not in config["pool"]
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("stai_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("stai_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("stai_target_address") or all_targets[-2] == config["pool"].get(
                    "stai_target_address"
                ):
        ...
        if "stai_target_address" not in config["farmer"]:
            print(
                f"Setting the stai destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
            config["farmer"]["stai_target_address"] = all_targets[0]
            updated_target = True
        elif config["farmer"]["stai_target_address"] not in all_targets:
            print(
                f"WARNING: using a farmer address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['farmer']['stai_target_address']} with {all_targets[0]}"
            )

        if "pool" not in config:
            config["pool"] = {}
        if "stai_target_address" not in config["pool"]:
            print(f"Setting the stai destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
            config["pool"]["stai_target_address"] = all_targets[0]
            updated_target = True
        elif config["pool"]["stai_target_address"] not in all_targets:
            print(
                f"WARNING: using a pool address which we might not have the private"
                f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
                f"{config['pool']['stai_target_address']} with {all_targets[0]}"
            )
        if updated_target:
            print(
                f"To change the STAI destination addresses, edit the `stai_target_address` entries in"
...
def init(
    ...
    if create_certs is not None:
        ...
        return stai_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)

    return None


def stai_version_number() -> Tuple[str, str, str, str]:
...
def stai_full_version_str() -> str:
    major, minor, patch, dev = stai_version_number()
...
def stai_init(
    ...
    *,
    ...
    """
    stai_root = os.environ.get("STAI_ROOT", None)
    if stai_root is not None:
        print(f"STAI_ROOT is set to {stai_root}")

    print(f"Stai directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if STAI_ROOT is set, or if user has run stai init twice
        # before a new update.
        if testnet:
            configure(
                root_path,
    ...
    print("")
    print("To see your keys, run 'stai keys show --show-mnemonic-seed'")
