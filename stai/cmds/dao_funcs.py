...
from stai.cmds.cmds_util import CMDTXConfigLoader, get_wallet_client, transaction_status_msg, transaction_submitted_msg
from stai.cmds.units import units
from stai.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type
...
from stai.wallet.util.tx_config import DEFAULT_COIN_SELECTION_CONFIG
...
async def create_dao_wallet(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    proposal_minimum = uint64(int(Decimal(args["proposal_minimum_amount"]) * units["stai"]))
    ...
    final_fee: uint64 = uint64(int(fee * units["stai"]))

    fee_for_cat = Decimal(args["fee_for_cat"])
    final_fee_for_cat: uint64 = uint64(int(fee_for_cat * units["stai"]))
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if len(conf_coins) < 2:  # pragma: no cover
            raise ValueError("DAO creation requires at least 2 stai coins in your wallet.")
        res = await wallet_client.create_new_dao_wallet(
            mode="new",
            dao_rules=dao_rules,
...
async def get_treasury_balance(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, _):
        ...
        stai_mojos = get_mojo_per_unit(WalletType.STANDARD_WALLET)
        cat_mojos = get_mojo_per_unit(WalletType.CAT)
        for asset_id, balance in balances.items():
            if asset_id == "stai":
                print(f"STAI: {balance / stai_mojos}")
            else:
...
async def show_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, _, config):
        ...
        if ptype == "spend":
            stai_conds = pd["stai_conditions"]
            asset_conds = pd["asset_conditions"]
            print("")
            if stai_conds:
                print("Proposal STAI Conditions")
                for pmt in stai_conds:
...
async def vote_on_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    fee = args["fee"]
    final_fee: uint64 = uint64(int(Decimal(fee) * units["stai"]))
    proposal_id = args["proposal_id"]
...
async def create_spend_proposal(args: Dict[str, Any], wallet_rpc_port: Optional[int], fp: int) -> None:
    ...
    async with get_wallet_client(wallet_rpc_port, fp) as (wallet_client, fingerprint, config):
        ...
        if res["success"]:
            asset_id_name = asset_id if asset_id else "STAI"
