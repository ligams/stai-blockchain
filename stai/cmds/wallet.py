...
from stai.cmds.check_wallet_db import help_text as check_help_text
from stai.cmds.coins import coins_cmd
from stai.cmds.plotnft import validate_fee
...
@wallet_cmd.command("send", help="Send stai to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@options.create_fingerprint()
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much stai to send, in XCH", type=str, required=True)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck Stai Wallet DBs"""

    from stai.cmds.check_wallet_db import scan
