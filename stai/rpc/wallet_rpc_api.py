...
from stai.data_layer.data_layer_errors import LauncherCoinNotFoundError
from stai.data_layer.data_layer_wallet import DataLayerWallet
from stai.pools.pool_wallet import PoolWallet
from stai.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from stai.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
from stai.rpc.util import tx_endpoint
...
from stai.types.blockchain_format.coin import Coin, coin_as_list
from stai.types.blockchain_format.program import Program
from stai.types.blockchain_format.sized_bytes import bytes32
from stai.types.coin_record import CoinRecord
from stai.types.coin_spend import CoinSpend
from stai.types.signing_mode import CHIP_0002_SIGN_MESSAGE_PREFIX, SigningMode
...
from stai.util.config import load_config, str2bool
from stai.util.errors import KeychainIsLocked
from stai.util.ints import uint16, uint32, uint64
from stai.util.keychain import bytes_to_mnemonic, generate_mnemonic
from stai.util.misc import UInt32Range
from stai.util.path import path_from_root
from stai.util.streamable import Streamable, streamable
from stai.util.ws_message import WsRpcMessage, create_payload_dict
from stai.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from stai.wallet.cat_wallet.cat_info import CRCATInfo
from stai.wallet.cat_wallet.cat_wallet import CATWallet
from stai.wallet.cat_wallet.dao_cat_info import LockedCoinInfo
from stai.wallet.cat_wallet.dao_cat_wallet import DAOCATWallet
from stai.wallet.conditions import Condition
from stai.wallet.dao_wallet.dao_info import DAORules
from stai.wallet.dao_wallet.dao_utils import (
...
from stai.wallet.dao_wallet.dao_wallet import DAOWallet
...
from stai.wallet.did_wallet import did_wallet_puzzles
from stai.wallet.did_wallet.did_info import DIDCoinData, DIDInfo
from stai.wallet.did_wallet.did_wallet import DIDWallet
from stai.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    did_program_to_metadata,
    match_did_puzzle,
    metadata_to_program,
)
from stai.wallet.nft_wallet import nft_puzzles
from stai.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from stai.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from stai.wallet.nft_wallet.nft_wallet import NFTWallet
from stai.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from stai.wallet.notification_store import Notification
from stai.wallet.outer_puzzles import AssetType
from stai.wallet.payment import Payment
from stai.wallet.puzzle_drivers import PuzzleInfo, Solver
from stai.wallet.puzzles import p2_delegated_conditions
from stai.wallet.puzzles.clawback.metadata import AutoClaimSettings, ClawbackMetadata
from stai.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from stai.wallet.singleton import (
    SINGLETON_LAUNCHER_PUZZLE_HASH,
    create_singleton_puzzle,
    get_inner_puzzle_from_singleton,
)
from stai.wallet.trade_record import TradeRecord
from stai.wallet.trading.offer import Offer
from stai.wallet.transaction_record import TransactionRecord
from stai.wallet.uncurried_puzzle import uncurry_puzzle
from stai.wallet.util.address_type import AddressType, is_valid_address
from stai.wallet.util.compute_hints import compute_spend_hints_and_additions
from stai.wallet.util.compute_memos import compute_memos
from stai.wallet.util.query_filter import HashFilter, TransactionTypeFilter
from stai.wallet.util.transaction_type import CLAWBACK_INCOMING_TRANSACTION_TYPES, TransactionType
from stai.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, CoinSelectionConfigLoader, TXConfig
from stai.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
from stai.wallet.util.wallet_types import CoinType, WalletType
from stai.wallet.vc_wallet.cr_cat_drivers import ProofsChecker
from stai.wallet.vc_wallet.cr_cat_wallet import CRCATWallet
from stai.wallet.vc_wallet.vc_store import VCProofs
from stai.wallet.vc_wallet.vc_wallet import VCWallet
from stai.wallet.wallet import Wallet
from stai.wallet.wallet_coin_record import WalletCoinRecord
from stai.wallet.wallet_coin_store import CoinRecordOrder, GetCoinRecords, unspent_range
from stai.wallet.wallet_info import WalletInfo
from stai.wallet.wallet_node import WalletNode
from stai.wallet.wallet_protocol import WalletProtocol
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSStaiConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("stai_target_address")
        pool_target = config["pool"].get("stai_target_address")
    ...
    async def create_new_wallet(
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                from stai.pools.pool_wallet_info import initial_pool_state_from_dict
    ...
    async def verify_signature(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        # signatures made by `stai keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request: Dict[str, Any]) -> EndpointResult:
        ...
        from stai.util.bech32m import bech32_decode, convertbits
        from stai.wallet.util.puzzle_compression import OFFER_MOD_OLD, decompress_object_with_puzzles
    ...
    async def cancel_offers(
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "stai")
        ...
        key = None
        if asset_id is not None and asset_id != "stai":
    ...
    async def nft_mint_bulk(
        ...
        stai_coin_list = request.get("stai_coins", None)
        stai_coins = None
        if stai_coin_list:
            stai_coins = {Coin.from_json_dict(stai_coin) for stai_coin in stai_coin_list}
        stai_change_target = request.get("stai_change_target", None)
        if stai_change_target is not None:
            if stai_change_target[:2] == "stai":
                stai_change_ph = decode_puzzle_hash(stai_change_target)
            else:
                stai_change_ph = bytes32(hexstr_to_bytes(stai_change_target))
        else:
            stai_change_ph = None
        ...
        if mint_from_did:
            txs = await nft_wallet.mint_from_did(
                ...
                stai_coins=stai_coins,
                stai_change_ph=stai_change_ph,
            ...
            txs = await nft_wallet.mint_from_stai(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
