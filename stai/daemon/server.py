...
from stai.cmds.init_funcs import check_keys, stai_full_version_str, stai_init
from stai.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from stai.consensus.coinbase import create_puzzlehash_for_pk
from stai.daemon.keychain_server import KeychainServer, keychain_commands
from stai.daemon.windows_signal import kill
from stai.plotters.plotters import get_available_plotters
from stai.plotting.util import add_plot_directory
from stai.server.server import ssl_context_for_server
from stai.util.bech32m import encode_puzzle_hash
from stai.util.beta_metrics import BetaMetricsLogger
from stai.util.stai_logging import initialize_service_logging
from stai.util.config import load_config
from stai.util.errors import KeychainCurrentPassphraseIsInvalid
from stai.util.ints import uint32
from stai.util.json_util import dict_to_json_str
from stai.util.keychain import Keychain, KeyData, passphrase_requirements, supports_os_passphrase_storage
from stai.util.lock import Lockfile, LockfileError
from stai.util.log_exceptions import log_exceptions
from stai.util.misc import SignalHandlers
from stai.util.network import WebServer
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Stai.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "stai": "stai",
        "stai_data_layer": "start_data_layer",
        "stai_data_layer_http": "start_data_layer_http",
        "stai_wallet": "start_wallet",
        "stai_full_node": "start_full_node",
        "stai_harvester": "start_harvester",
        "stai_farmer": "start_farmer",
        "stai_introducer": "start_introducer",
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["stai", "plotters", plotter]

        if plotter == "bladebit":
            # plotter command must be either
            # 'stai plotters bladebit ramplot' or 'stai plotters bladebit diskplot'
            plot_type = request["plot_type"]
...
def launch_plotter(
    ...
    # we need to pass on the possibly altered STAI_ROOT
    os.environ["STAI_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service {service_array} with STAI_ROOT: {os.environ['STAI_ROOT']}")
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in stai_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    stai_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("stai_daemon")
    ...
    try:
        with Lockfile.create(daemon_launch_lock_path(root_path), timeout=1):
            log.info(f"stai-blockchain version: {stai_full_version_str()}")
