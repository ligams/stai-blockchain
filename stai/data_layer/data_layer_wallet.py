...
from stai.data_layer.data_layer_errors import LauncherCoinNotFoundError, OfferIntegrityError
from stai.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from stai.types.spend_bundle import SpendBundle, estimate_fees
...
from stai.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from stai.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from stai.wallet.trading.offer import NotarizedPayment, Offer
from stai.wallet.transaction_record import TransactionRecord
from stai.wallet.util.compute_memos import compute_memos
from stai.wallet.util.merkle_utils import _simplify_merkle_proof
from stai.wallet.util.transaction_type import TransactionType
from stai.wallet.util.tx_config import CoinSelectionConfig, TXConfig, TXConfigLoader
from stai.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSStaiConnection) -> CoinState:
    ...
    async def delete_mirror(
        ...
        if excess_fee > 0:
            [stai_tx] = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
                tx_config,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, stai_tx.spend_bundle])
                ),
                dataclasses.replace(stai_tx, spend_bundle=None),
            ]
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSStaiConnection, coin_data: Optional[object]) -> None:
