...
from stai.types.coin_spend import CoinSpend, compute_additions, make_spend
...
from stai.util.condition_tools import conditions_dict_for_solution, pkm_pairs_for_conditions_dict
from stai.util.hash import std_hash
from stai.util.ints import uint16, uint32, uint64, uint128
from stai.wallet.conditions import Condition, UnknownCondition, parse_timelock_info
from stai.wallet.derivation_record import DerivationRecord
from stai.wallet.did_wallet import did_wallet_puzzles
from stai.wallet.did_wallet.did_info import DIDInfo
from stai.wallet.lineage_proof import LineageProof
from stai.wallet.nft_wallet import nft_puzzles
from stai.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from stai.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from stai.wallet.nft_wallet.uncurry_nft import NFTCoinData, UncurriedNFT
from stai.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from stai.wallet.payment import Payment
from stai.wallet.puzzle_drivers import PuzzleInfo, Solver
from stai.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from stai.wallet.trading.offer import OFFER_MOD, OFFER_MOD_HASH, NotarizedPayment, Offer
...
from stai.wallet.util.transaction_type import TransactionType
from stai.wallet.util.tx_config import CoinSelectionConfig, TXConfig
...
from stai.wallet.wallet_nft_store import WalletNftStore
from stai.wallet.wallet_protocol import GSTOptionalArgs, WalletProtocol
...
class NFTWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSStaiConnection, parent_coin_data: Optional[NFTCoinData]
    ) -> None:
    ...
    async def puzzle_solution_received(self, coin: Coin, data: NFTCoinData, peer: WSStaiConnection) -> None:
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        unsigned_spend_bundle, stai_tx = await self.generate_unsigned_spendbundle(
            payments,
            tx_config,
            fee,
            coins=coins,
        ...
        if stai_tx is not None and stai_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, stai_tx.spend_bundle])
            stai_tx = dataclasses.replace(stai_tx, spend_bundle=None)
        ...
        if stai_tx is not None:
            tx_list.append(stai_tx)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            stai_tx = await self.standard_wallet.create_tandem_stai_tx(
        ...
        return nft_spend_bundle, stai_tx
        ...
        for asset, amount in offer_dict.items():
            if amount < 0:
                ...
                if len(offered_coins) == 0:
                    raise ValueError(f"Did not have asset ID {asset.hex() if asset is not None else 'STAI'} to offer")
    ...
    async def mint_from_did(
        ...
        stai_coins: Optional[Set[Coin]] = None,
        stai_change_ph: Optional[bytes32] = None,
        ...
        - The launcher coins are then spent along with the created eve spend
          and an stai spend that funds the transactions and pays fees.
        ...
        :param stai_coins: [Optional] For use with bulk minting to provide the coin used for funding the minting spend.
        This coin can be one that will be created in the future
        :param stai_change_ph: [Optional] For use with bulk minting, so we can specify the puzzle hash that the change
        ...
        :param fee: A fee amount, taken out of the stai spend.
        """
        # get DID Wallet
        ...
        # Ensure we have an stai coin of high enough amount
        assert isinstance(fee, uint64)
        total_amount = len(metadata_list) + fee
        if stai_coins is None:
            stai_coins = await self.standard_wallet.select_coins(uint64(total_amount), tx_config.coin_selection_config)
        assert len(stai_coins) > 0
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            intermediate_launcher_puz = did_wallet_puzzles.INTERMEDIATE_LAUNCHER_MOD.curry(
                stai.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, mint_number, mint_total
            )
            ...
            launcher_coin = Coin(
                intermediate_launcher_coin.name(), stai.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE_HASH, amount
            )
            ...
            launcher_cs = make_spend(
                launcher_coin, stai.wallet.singleton.SINGLETON_LAUNCHER_PUZZLE, genesis_launcher_solution
            )
        ...
        # Create the stai spend to fund the minting.
        spend_value = sum([coin.amount for coin in stai_coins])
        change: uint64 = uint64(spend_value - total_amount)
        stai_spends = []
        if stai_change_ph is None:
            stai_change_ph = await self.standard_wallet.get_new_puzzlehash()
        stai_payment = Payment(stai_change_ph, change, [stai_change_ph])

        first = True
        for stai_coin in stai_coins:
            puzzle: Program = await self.standard_wallet.puzzle_for_puzzle_hash(stai_coin.puzzle_hash)
            if first:
                message_list: List[bytes32] = [c.name() for c in stai_coins]
                message_list.append(Coin(stai_coin.name(), stai_payment.puzzle_hash, stai_payment.amount).name())
                message: bytes32 = std_hash(b"".join(message_list))

                if len(stai_coins) > 1:
                    stai_announcement: Optional[Set[bytes]] = {message}
                else:
                    stai_announcement = None

                solution: Program = self.standard_wallet.make_solution(
                    primaries=[stai_payment],
                    fee=fee,
                    coin_announcements=stai_announcement,
                    coin_announcements_to_assert={Announcement(did_coin.name(), message).name()},
                )
                primary_announcement_hash = Announcement(stai_coin.name(), message).name()
            ...
            stai_spends.append(make_spend(stai_coin, puzzle, solution))
        stai_spend = await self.wallet_state_manager.sign_transaction(stai_spends)
        ...
        did_full_puzzle: Program = stai.wallet.singleton.create_singleton_puzzle(
            innerpuz,
        ...
        total_spend = SpendBundle.aggregate([signed_spend_bundle, stai_spend, *eve_spends])

        tx_record: TransactionRecord = dataclasses.replace(eve_txs[0], spend_bundle=total_spend)
        return [tx_record]

    async def mint_from_stai(
        self,
        metadata_list: List[Dict[str, Any]],
        tx_config: TXConfig,
        target_list: Optional[List[bytes32]] = [],
        ...
        funding_coin = stai_coins.copy().pop()
        ...
        for mint_number in range(mint_number_start, mint_number_end):
            ...
            # Annnouncements for eve spend. These are asserted by the stai spend
            announcement_message = Program.to([eve_fullpuz.get_tree_hash(), amount, []]).get_tree_hash()
        ...
        for stai_coin in stai_coins:
            ...
            if first:
                ...
                solution: Program = self.standard_wallet.make_solution(
                    primaries=[stai_payment] + primaries,
                    fee=fee,
                    coin_announcements=stai_announcement if len(stai_coins) > 1 else None,
