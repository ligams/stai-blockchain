...
from stai.protocols.wallet_protocol import CoinState, RequestBlockHeader, RespondBlockHeader
...
from stai.wallet import singleton
from stai.wallet.cat_wallet.cat_utils import CAT_MOD, SpendableCAT, construct_cat_puzzle
from stai.wallet.cat_wallet.cat_utils import get_innerpuzzle_from_puzzle as get_innerpuzzle_from_cat_puzzle
from stai.wallet.cat_wallet.cat_utils import unsigned_spend_bundle_for_spendable_cats
...
from stai.wallet.conditions import Condition, parse_timelock_info
from stai.wallet.dao_wallet.dao_info import DAOInfo, DAORules, ProposalInfo, ProposalType
...
from stai.wallet.util.tx_config import DEFAULT_TX_CONFIG, CoinSelectionConfig, TXConfig
from stai.wallet.util.wallet_sync_utils import fetch_coin_spend
...
class DAOWallet:
    ...
    It is not a user-facing wallet. A user cannot spend or receive STAI though a wallet of this type.
    ...
    # if asset_id == None: then we get normal STAI
    ...
    async def coin_added(self, coin: Coin, height: uint32, peer: WSStaiConnection, coin_data: Optional[Any]) -> None:
        """
        ...
        try:
            ...
            if matched_funding_puz:
                # funding coin
                stai_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
                if coin.puzzle_hash == stai_funds_puzhash:
                    asset_id = None
                else:
    ...
    async def resync_treasury_state(self) -> None:
        ...
        wallet_node: Any = self.wallet_state_manager.wallet_node
        peer: WSStaiConnection = wallet_node.get_full_node_peer()
        if peer is None:  # pragma: no cover
            raise ValueError("Could not find any peers to request puzzle and solution from")
        ...
        parent_inner_puz = stai.wallet.singleton.get_inner_puzzle_from_singleton(
        ...
        # get existing stai funds for treasury
        stai_funds_puzhash = get_p2_singleton_puzhash(self.dao_info.treasury_id, asset_id=None)
        await self.wallet_state_manager.add_interested_puzzle_hashes([stai_funds_puzhash], [self.id()])
    ...
    async def generate_proposal_vote_spend(
        ...
        if fee > 0:
            ...
            spend_bundle = unsigned_spend_bundle.aggregate([unsigned_spend_bundle, dao_cat_spend, stai_tx.spend_bundle])
        else:
    ...
    async def create_proposal_close_spend(
        ...
        if proposal_state["passed"] and not self_destruct:
            ...
            if proposal_type == ProposalType.SPEND:
                ...
                if sum > 0:
                    stai_coins = await self.select_coins_for_asset_type(uint64(sum))
                    for stai_coin in stai_coins:
                        stai_parent_amount_list.append([stai_coin.parent_coin_info, stai_coin.amount])
                        solution = Program.to(
                            [
                                0,
                                treasury_inner_puzhash,
                        ...
                        coin_spends.append(make_spend(stai_coin, p2_singleton_puzzle, solution))
        ...
        if fee > 0:
            stai_tx = await self.standard_wallet.create_tandem_stai_tx(fee, tx_config)
            assert stai_tx.spend_bundle is not None
            full_spend = SpendBundle.aggregate([spend_bundle, stai_tx.spend_bundle])
        else:
    ...
    async def free_coins_from_finished_proposals(
        ...
        if fee > 0:
            stai_tx = await self.standard_wallet.create_tandem_stai_tx(fee, tx_config)
            assert stai_tx.spend_bundle is not None
            full_spend = full_spend.aggregate([full_spend, stai_tx.spend_bundle])
            ...
            if prop_info.proposal_id == proposal_id:
                ...
                if proposal_type == ProposalType.SPEND:
                    ...
                    dictionary: Dict[str, Any] = {
                        ...
                        "stai_conditions": stai_created_coins,
