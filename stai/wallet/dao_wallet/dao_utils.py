...
from stai.wallet.cat_wallet.cat_utils import CAT_MOD, CAT_MOD_HASH, construct_cat_puzzle
from stai.wallet.dao_wallet.dao_info import DAORules, ProposalType
from stai.wallet.puzzles.load_clvm import load_clvm
from stai.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import MOD
from stai.wallet.singleton import get_singleton_struct_for_id
from stai.wallet.uncurried_puzzle import UncurriedPuzzle
...
DAO_CAT_TAIL: Program = load_clvm(
    "genesis_by_coin_id_or_singleton.clsp", package_or_requirement="stai.wallet.cat_wallet.puzzles"
)
...
def get_spend_p2_singleton_puzzle(treasury_id: bytes32, stai_conditions: Program, asset_conditions: Program) -> Program:
    # TODO: typecheck get_spend_p2_singleton_puzzle arguments
    # TODO: add tests for get_spend_p2_singleton_puzzle: pass stai_conditions as Puzzle, List and ConditionWithArgs
    #

    # CAT_MOD_HASH
    # CONDITIONS  ; STAI conditions, to be generated by the treasury
...
def generate_simple_proposal_innerpuz(
    ...
    for recipient_puzhash, amount, asset_type in zip(recipient_puzhashes, amounts, asset_types):
        if asset_type:
            ...
            stai_conds.append([51, recipient_puzhash, amount])
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(stai_conds), Program.to(cat_conds))
    return puzzle
...
async def generate_mint_proposal_innerpuz(
    ...
    puzzle = get_spend_p2_singleton_puzzle(treasury_id, Program.to(stai_conditions), Program.to([]))
    return puzzle
