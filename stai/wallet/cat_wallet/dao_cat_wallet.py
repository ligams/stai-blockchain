...
from stai.wallet.cat_wallet.dao_cat_info import DAOCATInfo, LockedCoinInfo
...
from stai.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash
...
class DAOCATWallet:
    ...
    async def remove_active_proposal(
        ...
        if fee > 0:  # pragma: no cover
            stai_tx = await self.standard_wallet.create_tandem_stai_tx(fee, tx_config=tx_config)
            assert stai_tx.spend_bundle is not None
            full_spend = SpendBundle.aggregate([spend_bundle, stai_tx.spend_bundle])
        else:
            full_spend = spend_bundle
