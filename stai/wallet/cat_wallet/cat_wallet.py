...
from stai.types.coin_spend import compute_additions_with_cost
...
from stai.util.errors import Err, ValidationError
...
from stai.wallet.cat_wallet.cat_info import CATCoinData, CATInfo, LegacyCATInfo
...
from stai.wallet.cat_wallet.lineage_store import CATLineageStore
from stai.wallet.coin_selection import select_coins
from stai.wallet.conditions import Condition, ConditionValidTimes, UnknownCondition, parse_timelock_info
...
from stai.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
from stai.wallet.util.curry_and_treehash import calculate_hash_of_quoted_mod_hash, curry_and_treehash
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            stai_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        stai_tx = dataclasses.replace(stai_tx, spend_bundle=spend_bundle, name=spend_bundle.name())
        await self.standard_wallet.push_transaction(stai_tx)
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSStaiConnection, parent_coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def create_tandem_stai_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            stai_coins = await self.standard_wallet.select_coins(
                fee,
                tx_config.coin_selection_config,
            )
            origin_id = list(stai_coins)[0].name()
            [stai_tx] = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
            ...
            origin_id = list(stai_coins)[0].name()
            selected_amount = sum([c.amount for c in stai_coins])
            [stai_tx] = await self.standard_wallet.generate_signed_transaction(
            ...
            message = None
            for spend in stai_tx.spend_bundle.coin_spends:
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some STAI as part of this
        regular_stai_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_stai_to_claim = payment_amount

        need_stai_transaction = (fee > 0 or regular_stai_to_claim > 0) and (fee - regular_stai_to_claim != 0)
        ...
        for coin in cat_coins:
            ...
            if first:
                ...
                if need_stai_transaction:
                    if fee > regular_stai_to_claim:
                        stai_tx, _ = await self.create_tandem_stai_tx(
                            fee,
                            uint64(regular_stai_to_claim),
                            tx_config,
                    ...
                    elif regular_stai_to_claim > fee:
                        stai_tx, _ = await self.create_tandem_stai_tx(
                            fee,
                            uint64(regular_stai_to_claim),
                            tx_config,
        ...
        stai_spend_bundle = SpendBundle([], G2Element())
        if stai_tx is not None and stai_tx.spend_bundle is not None:
            stai_spend_bundle = stai_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if stai_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=stai_tx.confirmed_at_height,
                    created_at_time=stai_tx.created_at_time,
                    to_puzzle_hash=stai_tx.to_puzzle_hash,
                    amount=stai_tx.amount,
                    fee_amount=stai_tx.fee_amount,
                    confirmed=stai_tx.confirmed,
                    sent=stai_tx.sent,
                    spend_bundle=None,
                    additions=stai_tx.additions,
                    removals=stai_tx.removals,
                    wallet_id=stai_tx.wallet_id,
                    sent_to=stai_tx.sent_to,
                    trade_id=stai_tx.trade_id,
                    type=stai_tx.type,
                    name=stai_tx.name,
                    memos=[],
                    valid_times=parse_timelock_info(extra_conditions),
