...
from stai.consensus.blockchain import AddBlockResult
from stai.consensus.constants import ConsensusConstants
from stai.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from stai.full_node.full_node_api import FullNodeAPI
from stai.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from stai.protocols.protocol_message_types import ProtocolMessageTypes
from stai.protocols.wallet_protocol import (
    CoinState,
    CoinStateUpdate,
    NewPeakWallet,
...
from stai.rpc.rpc_server import StateChangedProtocol, default_get_connections
from stai.server.node_discovery import WalletPeers
...
from stai.util.config import (
    WALLET_PEERS_PATH_KEY_DEPRECATED,
    lock_and_load_config,
    process_config_start_method,
    save_config,
)
from stai.util.db_wrapper import manage_connection
from stai.util.errors import KeychainIsEmpty, KeychainIsLocked, KeychainKeyNotFound, KeychainProxyConnectionFailure
...
from stai.util.misc import to_batches
...
from stai.wallet.puzzles.clawback.metadata import AutoClaimSettings
from stai.wallet.transaction_record import TransactionRecord
from stai.wallet.util.new_peak_queue import NewPeakItem, NewPeakQueue, NewPeakQueueTypes
from stai.wallet.util.peer_request_cache import PeerRequestCache, can_use_peer_request_cache
...
from stai.wallet.wallet_weight_proof_handler import WalletWeightProofHandler, get_wp_fork_point
...
class WalletNode:
    ...
    def get_cache_for_peer(self, peer: WSStaiConnection) -> PeerRequestCache:
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'stai keys' program.")
            return None
    ...
    def initialize_wallet_peers(self) -> None:
        ...
        if self.wallet_peers is None and connect_to_unknown_peers and not testing:
            self.wallet_peers = WalletPeers(
                ...
                self.config.get("dns_servers", ["dns-introducer.stai.net"]),
    ...
    async def on_disconnect(self, peer: WSStaiConnection) -> None:
    ...
    async def on_connect(self, peer: WSStaiConnection) -> None:
        if self._wallet_state_manager is None:
            return None
    ...
    def is_trusted(self, peer: WSStaiConnection) -> bool:
        return self.server.is_trusted_peer(peer, self.config.get("trusted_peers", {}))

    async def state_update_received(self, request: CoinStateUpdate, peer: WSStaiConnection) -> None:
    ...
    def get_full_node_peer(self) -> WSStaiConnection:
        """
        Get a full node, preferring synced & trusted > synced & untrusted > unsynced & trusted > unsynced & untrusted
        """
        full_nodes: List[WSStaiConnection] = self.get_full_node_peers_in_order()
        if len(full_nodes) == 0:
            raise ValueError("No peer connected")
        return full_nodes[0]

    def get_full_node_peers_in_order(self) -> List[WSStaiConnection]:
        """
        ...
        synced_and_trusted: List[WSStaiConnection] = []
        synced: List[WSStaiConnection] = []
        trusted: List[WSStaiConnection] = []
        neither: List[WSStaiConnection] = []
        all_nodes: List[WSStaiConnection] = self.server.get_connections(NodeType.FULL_NODE)
    ...
    async def get_timestamp_for_height_from_peer(self, height: uint32, peer: WSStaiConnection) -> Optional[uint64]:
        """
    ...
    async def new_peak_wallet(self, new_peak: NewPeakWallet, peer: WSStaiConnection) -> None:
        if self._wallet_state_manager is None:
    ...
    async def new_peak_from_trusted(
        self, new_peak_hb: HeaderBlock, latest_timestamp: uint64, peer: WSStaiConnection
    ) -> None:
    ...
    async def new_peak_from_untrusted(self, new_peak_hb: HeaderBlock, peer: WSStaiConnection) -> bool:
        far_behind: bool = (
    ...
    async def long_sync_from_untrusted(self, syncing: bool, new_peak_hb: HeaderBlock, peer: WSStaiConnection) -> None:
    ...
    async def sync_from_untrusted_close_to_peak(self, new_peak_hb: HeaderBlock, peer: WSStaiConnection) -> bool:
        async with self.wallet_state_manager.lock:
    ...
    async def wallet_short_sync_backtrack(self, header_block: HeaderBlock, peer: WSStaiConnection) -> int:
    ...
    async def fetch_and_update_weight_proof(self, peer: WSStaiConnection, peak: HeaderBlock) -> int:
    ...
    async def validate_block_inclusion(
        self, block: HeaderBlock, peer: WSStaiConnection, peer_request_cache: PeerRequestCache
    ) -> bool:
    ...
    async def get_coin_state(
        self, coin_names: List[bytes32], peer: WSStaiConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
    ...
    async def fetch_children(
        self, coin_name: bytes32, peer: WSStaiConnection, fork_height: Optional[uint32] = None
    ) -> List[CoinState]:
