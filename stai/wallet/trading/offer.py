...
from stai.util.bech32m import bech32_decode, bech32_encode, convertbits
...
from stai.wallet.outer_puzzles import (
    construct_puzzle,
    create_asset_id,
    get_inner_puzzle,
    get_inner_solution,
...
from stai.wallet.uncurried_puzzle import UncurriedPuzzle, uncurry_puzzle
from stai.wallet.util.compute_hints import compute_spend_hints_and_additions
from stai.wallet.util.puzzle_compression import (
...
class Offer:
    ...
    def notarize_payments(
        requested_payments: Dict[Optional[bytes32], List[Payment]],  # `None` means you are requesting STAI
        coins: List[Coin],
    ...
    def _get_offered_coins(self) -> Dict[Optional[bytes32], List[Coin]]:
        ...
        for parent_spend in self._bundle.coin_spends:
            ...
            if puzzle_driver is not None:
                ...
                # It's much easier if the asset is bare STAI
                asset_id = None
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "stai" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
