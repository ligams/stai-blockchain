...
recompile_requested = (
    (os.environ.get("STAI_DEV_COMPILE_CLVM_ON_IMPORT", "") != "") or ("pytest" in sys.modules)
) and os.environ.get("STAI_DEV_COMPILE_CLVM_DISABLED", None) is None
...
def load_serialized_clvm(
    ...
    # Set the STAI_DEV_COMPILE_CLVM_ON_IMPORT environment variable to anything except
            ...
            if pkg_resources.resource_exists(package_or_requirement, clvm_filename):
                ...
                if not output.exists() or os.stat(full_path).st_mtime > os.stat(output).st_mtime:
                    ...
                    if include_standard_libraries:
                        ...
                        stai_puzzles_path = pathlib.Path(
                            pkg_resources.resource_filename(__name__, "__init__.py")
                        ).parent
                        search_paths.append(stai_puzzles_path)
