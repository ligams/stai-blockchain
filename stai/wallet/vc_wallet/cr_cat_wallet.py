...
from stai.util.misc import VersionedBlob
from stai.wallet.cat_wallet.cat_info import CATCoinData, CRCATInfo
from stai.wallet.cat_wallet.cat_utils import CAT_MOD_HASH, CAT_MOD_HASH_HASH, construct_cat_puzzle
...
from stai.wallet.util.tx_config import TXConfig
...
from stai.wallet.vc_wallet.cr_cat_drivers import (
    CRCAT,
    CRCATMetadata,
    CRCATVersion,
    ProofsChecker,
...
class CRCATWallet(CATWallet):
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSStaiConnection, coin_data: Optional[CATCoinData]
    ) -> None:
    ...
    async def _generate_unsigned_spendbundle(
            ...
            if first:
                ...
                if need_stai_transaction:
                    if fee > regular_stai_to_claim:
                        ...
                        stai_tx, stai_announcement = await self.create_tandem_stai_tx(
                            fee,
                            uint64(regular_stai_to_claim),
                            tx_config,
                        )
                        assert stai_announcement is not None
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
                            coin_announcements_to_assert={stai_announcement.name()},
                            conditions=extra_conditions,
                        )
                else:
                    innersol = self.standard_wallet.make_solution(
        ...
        return (
            SpendBundle(
                [
                    ...
                    *(
                        (
                            spend
                            for bundle in [stai_tx.spend_bundle]
            ...
            [*vc_txs, *([stai_tx] if stai_tx is not None else [])],
        )
    ...
    async def claim_pending_approval_balance(
        ...
        # Select the relevant STAI coins
        if fee > 0:
        ...
        nonce: bytes32 = Program.to(
            [coin_as_list(c) for c in sorted(coins.union(stai_coins).union({vc.coin}), key=Coin.name)]
        ).get_tree_hash()
        ...
        if fee > 0:
            ...
            if stai_tx.spend_bundle is None:
                raise RuntimeError("Did not get spendbundle for fee transaction")  # pragma: no cover
            claim_bundle = SpendBundle.aggregate([claim_bundle, stai_tx.spend_bundle])
        else:
            stai_tx = None
        ...
        other_txs: List[TransactionRecord] = [
            ...
            *((dataclasses.replace(stai_tx, spend_bundle=None),) if stai_tx is not None else []),
        ]
