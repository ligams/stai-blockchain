...
from stai.wallet.puzzle_drivers import Solver
from stai.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import solution_for_conditions
...
from stai.wallet.vc_wallet.cr_cat_drivers import CRCAT, CRCATSpend, ProofsChecker, construct_pending_approval_state
from stai.wallet.vc_wallet.vc_drivers import VerifiedCredential
from stai.wallet.vc_wallet.vc_store import VCProofs, VCRecord, VCStore
...
if TYPE_CHECKING:
    from stai.wallet.wallet_state_manager import WalletStateManager  # pragma: no cover
...
class VCWallet:
    ...
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSStaiConnection, coin_data: Optional[Streamable]
    ) -> None:
        """
    ...
    async def generate_signed_transaction(
        ...
        **kwargs: Unpack[GSTOptionalArgs],
        ...
        if fee > 0:
            ...
            stai_tx = await self.wallet_state_manager.main_wallet.create_tandem_stai_tx(
        ...
        if stai_tx is not None and stai_tx.spend_bundle is not None:
            spend_bundles.append(stai_tx.spend_bundle)
            tx_list.append(dataclasses.replace(stai_tx, spend_bundle=None))
    ...
    async def revoke_vc(
        ...
        if fee > 0:
            stai_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.create_tandem_stai_tx(
                fee, tx_config, vc_announcement
            )
            assert did_tx.spend_bundle is not None
            assert stai_tx.spend_bundle is not None
            new_bundle = SpendBundle.aggregate([stai_tx.spend_bundle, did_tx.spend_bundle])
