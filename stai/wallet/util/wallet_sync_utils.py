...
from stai.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from stai.wallet.util.peer_request_cache import PeerRequestCache
...
async def request_and_validate_removals(
    peer: WSStaiConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSStaiConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[Tuple[WSStaiConnection, bool]],
...
async def fetch_coin_spend(height: uint32, coin: Coin, peer: WSStaiConnection) -> CoinSpend:
...
async def fetch_coin_spend_for_coin_state(coin_state: CoinState, peer: WSStaiConnection) -> CoinSpend:
