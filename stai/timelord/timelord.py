...
from staivdf import create_discriminant, prove
...
from stai.timelord.iters_from_block import iters_from_block
from stai.timelord.timelord_state import LastState
from stai.timelord.types import Chain, IterationType, StateType
from stai.types.blockchain_format.classgroup import ClassgroupElement
from stai.types.blockchain_format.reward_chain_block import RewardChainBlock
...
from stai.types.blockchain_format.vdf import VDFInfo, VDFProof, validate_vdf
from stai.types.end_of_slot_bundle import EndOfSubSlotBundle
from stai.util.config import process_config_start_method
...
class Timelord:
    ...
    def __init__(self, root_path: Path, config: Dict[str, Any], constants: ConsensusConstants) -> None:
        ...
        self._server: Optional[StaiServer] = None
    ...
    async def manage(self) -> AsyncIterator[None]:
        ...
        if not self.bluebox_mode:
            ...
            if os.name == "nt" or slow_bluebox:
                # `vdf_client` doesn't build on windows, use `prove()` from staivdf.
