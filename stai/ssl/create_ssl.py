...
from stai.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def get_stai_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "stai_ca.crt")
    key = pkg_resources.resource_string(__name__, "stai_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Stai"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Stai"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("stai.net")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Stai"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Stai CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def create_all_ssl(
    ...
    *,
    ...
    stai_ca_crt, stai_ca_key = get_stai_ca_crt_key()
    stai_ca_crt_path = ca_dir / "stai_ca.crt"
    stai_ca_key_path = ca_dir / "stai_ca.key"
    write_ssl_cert_and_key(stai_ca_crt_path, stai_ca_crt, stai_ca_key_path, stai_ca_key, overwrite=overwrite)
...
def main():
    return make_ca_cert(Path("./stai_ca.crt"), Path("./stai_ca.key"))
