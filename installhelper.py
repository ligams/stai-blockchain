...
# Gets the version from the result of "stai version"
...
def get_stai_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["stai", "version"], capture_output=True)
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/stai-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/stai-blockchain-gui/packages/gui/package.json")
