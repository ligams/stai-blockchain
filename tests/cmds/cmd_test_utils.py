...
import stai.cmds.wallet_funcs
from stai.cmds.stai import cli as stai_cli
from stai.cmds.cmds_util import _T_RpcClient, node_config_section_names
...
class TestWalletRpcClient(TestRpcClient):
    ...
    async def get_next_address(self, wallet_id: int, new_address: bool) -> str:
        ...
        addr = encode_puzzle_hash(bytes32([self.wallet_index] * 32), "stai")
        self.wallet_index += 1
...
def create_service_and_wallet_client_generators(test_rpc_clients: TestRpcClients, default_root: Path) -> None:
    ...
    These are monkey patched into the stai.cmds.cmds_util module.
    ...
    stai.cmds.cmds_util.get_any_service_client = test_get_any_service_client
    stai.cmds.cmds_util.get_wallet_client = test_get_wallet_client  # type: ignore[assignment]
    stai.cmds.wallet_funcs.get_wallet_client = test_get_wallet_client  # type: ignore[assignment,attr-defined]
    # Monkey patches the confirm function to not ask for confirmation
    stai.cmds.cmds_util.cli_confirm = cli_confirm
    stai.cmds.wallet_funcs.cli_confirm = cli_confirm  # type: ignore[attr-defined]


def run_cli_command(capsys: object, stai_root: Path, command_list: List[str]) -> str:
    """
    This is just an easy way to run the stai CLI with the given command list.
    """
    # we don't use the real capsys object because its only accessible in a private part of the pytest module
    exited_cleanly = True
    argv_temp = sys.argv
    try:
        sys.argv = ["stai", "--root-path", str(stai_root)] + command_list
        stai_cli()  # pylint: disable=no-value-for-parameter
...
def run_cli_command_and_assert(
    capsys: object, stai_root: Path, command_list: List[str], strings_to_assert: Iterable[str]
) -> None:
    """
    Runs the command and asserts that all the strings in strings_to_assert are in the output
    """
    output = run_cli_command(capsys, stai_root, command_list)
