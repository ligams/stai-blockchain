...
std_farming_address = "tstai1mh4qanzyawn3v4uphgaj2cg6hrjazwyp0sx653fhn9apg6mfajlqtj0ztp"
burn_address = "tstai1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqm6ksh7qddh"  # 0x0...dead
...
def test_every_simulator_command() -> None:
    ...
    assert "stai_full_node_simulator: started" in start_result.output
...
def stop_simulator(runner: CliRunner, simulator_name: str) -> None:
    ...
    assert "stai_full_node_simulator: Stopped\nDaemon stopped\n" == result.output
...
def _test_sim_status(runner: CliRunner, address: str, simulator_name: str) -> None:
    ...
    assert f"Current Farming address: {address}, with a balance of: 21000000.0 TSTAI." in result.output  # default
