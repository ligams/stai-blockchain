...
from stai.wallet.conditions import parse_timelock_info
...
def test_dao_treasury(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class DAOCreateRpcClient(TestWalletRpcClient):
        ...
        async def dao_get_treasury_balance(self, wallet_id: int) -> Dict[str, Union[str, bool, Dict[str, int]]]:
            if wallet_id == 2:
                return {"success": True, "balances": {"stai": 1000000000000, "0xCAFEF00D": 10000000}}
            else:
    ...
    get_balance_asserts = ["STAI: 1.0", "0xCAFEF00D: 10000.0"]
...
def test_dao_proposals(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
        ...
        async def dao_parse_proposal(
            ...
            if proposal_id == "0xCAFEF00D":
                ...
                proposal_details: Dict[str, Any] = {
                    "proposal_type": "s",
                    "stai_conditions": [{"puzzle_hash": puzhash, "amount": 100}],
                    "asset_conditions": [
    ...
    address = encode_puzzle_hash(bytes32(b"1" * 32), "stai")
    asset_id = bytes32(b"2" * 32).hex()
    parse_spend_asserts = [
        "Type: SPEND",
        "Status: OPEN",
    ...
    parse_mint_asserts = [
        ...
        "Address: {}".format(encode_puzzle_hash(bytes32(b"x" * 32), "stai")),
    ]
    ...
    address = encode_puzzle_hash(bytes32(b"x" * 32), "stai")
    spend_args = [
        "dao",
        "create_proposal",
        "spend",
