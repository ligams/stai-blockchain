...
from stai.util.ints import uint8, uint32, uint64
from stai.wallet.conditions import ConditionValidTimes
...
from stai.wallet.util.tx_config import DEFAULT_TX_CONFIG, TXConfig
...
def test_get_transaction(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Status: In mempool",
        "Amount sent: 0.000012345678 STAI",
        "To address: stai1qyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqs0wg4qq",
    ]
    v_assert_list = [
...
def test_get_transactions(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        ...
        "Amount received: 0.00001234568 STAI",
        "Amount received in clawback as sender: 0.000012345681 STAI",
        "To address: stai1qvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvps82kgr2",
        "To address: stai1qszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqgpqyqszqkxck8d",
    ]
    v_assert_list = [
        "'amount': 12345680",
        "'fee_amount': 1234569",
...
def test_show(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class ShowRpcClient(TestWalletRpcClient):
        async def get_wallets(self, wallet_type: Optional[WalletType] = None) -> List[Dict[str, Union[str, int]]]:
            ...
            wallet_list: List[Dict[str, Union[str, int]]] = [
                {"data": "", "id": 1, "name": "Stai Wallet", "type": WalletType.STANDARD_WALLET},
                {
    ...
    assert_list = [
        "Stai Wallet:\n   -Total Balance:         0.001 stai (1000000000 mojo)",
...
def test_send(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    # get output with all options but verbose
    addr = encode_puzzle_hash(get_bytes32(3), "stai")
    command_args = [
        "wallet",
        "send",
        "-a1",
    ...
    expected_calls: logType = {
        ...
        "send_transaction": [
            (
                1,
                1000000000000,
                "stai1qvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvpsxqcrqvps82kgr2",
                TXConfig(
...
def test_get_address(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class GetAddressWalletRpcClient(TestWalletRpcClient):
        async def get_next_address(self, wallet_id: int, new_address: bool) -> str:
            ...
            if new_address:
                return encode_puzzle_hash(get_bytes32(3), "stai")
            return encode_puzzle_hash(get_bytes32(4), "stai")

    inst_rpc_client = GetAddressWalletRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    # get output with all options but verbose
    addr1 = encode_puzzle_hash(get_bytes32(3), "stai")
    addr2 = encode_puzzle_hash(get_bytes32(4), "stai")
    command_args = [
        "wallet",
        "get_address",
        WALLET_ID_ARG,
...
def test_sign_message(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    inst_rpc_client = TestWalletRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    stai_addr = encode_puzzle_hash(get_bytes32(1), "stai")
    ...
    run_cli_command_and_assert(capsys, root_dir, command_args + [f"-a{stai_addr}"], assert_list)
    expected_calls: logType = {
        "sign_message_by_address": [(stai_addr, message.hex())],  # stai std
    }
    test_rpc_clients.wallet_rpc_client.check_log(expected_calls)
...
def test_make_offer(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path], tmp_path: Path) -> None:
    ...
    # we offer stai and a random cat via wallet id and request a random cat, nft via coin and tail
    command_args = [
        "wallet",
        "make_offer",
        FINGERPRINT_ARG,
    ...
    expected_calls: logType = {
        ...
        "nft_calculate_royalties": [
            (
                ...
                {"STAI": 10000000000000, "test3": 100000},
            )
        ],
        "create_offer_for_ids": [
            (
...
def test_take_offer(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    assert_list = [
        "  OFFERED:\n"
        "    - STAI (Wallet ID: 1): 10.0 (10000000000000 mojos)\n"
