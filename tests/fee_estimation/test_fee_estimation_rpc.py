...
from stai.types.aliases import SimulatorFullNodeService, WalletService
...
async def test_validate_fee_estimate_cost_err(
    ...
    bad_arglist: List[List[Any]] = [
        ...
        [["spend_bundle", spend_bundle.to_json_dict()], ["spend_type", "send_stai_transaction"]],
        [["cost", 1], ["spend_type", "send_stai_transaction"]],
        [["spend_bundle", spend_bundle.to_json_dict()], ["cost", 1], ["spend_type", "send_stai_transaction"]],
    ]
...
async def test_validate_fee_estimate_cost_ok(
    ...
    good_arglist: List[List[Any]] = [
        ...
        ["cost", 1],
        ["spend_type", "send_stai_transaction"],
    ]
...
async def test_get_spendbundle_type_cost_spend_count_ok(
    ...
    for spend_count in spend_counts:
        request = {"target_times": [1], "spend_type": "send_stai_transaction", "spend_count": spend_count}
...
async def test_get_spendbundle_type_cost_spend_count_bad(
    ...
    client, full_node_rpc_api = setup_node_and_rpc
    with pytest.raises(ValueError):
        request = {"target_times": [1], "spend_type": "send_stai_transaction", "spend_count": -1}
        _ = await full_node_rpc_api.get_fee_estimate(request)
