...
from stai.wallet.util.debug_spend_bundle import disassemble
...
async def test_nft_offer_with_fee(
    ...
    # MAKE FIRST TRADE: 1 NFT for 100 stai
    ...
    stai_request = 100
    maker_fee = uint64(10)
    offer_nft_for_stai = {wallet_maker.id(): stai_request, nft_asset_id: -1}
    maker_unused_index = (
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_stai, tx_config, driver_dict, fee=maker_fee
    )
    ...
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre + stai_request - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre - stai_request - taker_fee)
    ...
    # MAKE SECOND TRADE: 100 stai for 1 NFT
    ...
    stai_offered = 1000
    maker_fee = uint64(10)
    offer_stai_for_nft = {wallet_maker.id(): -stai_offered, nft_to_buy_asset_id: 1}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_stai_for_nft, tx_config, driver_dict_to_buy, fee=maker_fee
    )
    assert success is True
    assert error is None
    assert trade_make is not None
    ...
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_maker, trade_make)
    await time_out_assert(20, get_trade_and_status, TradeStatus.CONFIRMED, trade_manager_taker, trade_take)
    await time_out_assert(20, wallet_maker.get_confirmed_balance, maker_balance_pre - stai_offered - maker_fee)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, taker_balance_pre + stai_offered - taker_fee)
...
async def test_nft_offer_cancellations(
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_nft_for_stai, DEFAULT_TX_CONFIG, driver_dict, fee=maker_fee
    )
    assert success is True
    assert error is None
    assert trade_make is not None
...
async def test_nft_offer_nft_for_nft(
    ...
    metadata_2 = Program.to(
        [
            ("u", ["https://www.stai.net/image2.html"]),
