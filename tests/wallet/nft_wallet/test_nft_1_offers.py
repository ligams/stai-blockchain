...
from stai.wallet.outer_puzzles import create_asset_id, match_puzzle
...
async def test_nft_offer_sell_nft(
    ...
    # maker create offer: NFT for stai
    ...
    offer_did_nft_for_stai = {nft_to_offer_asset_id: -1, wallet_maker.id(): stai_requested}

    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        offer_did_nft_for_stai, DEFAULT_TX_CONFIG, {}, fee=maker_fee
    )
    ...
    expected_royalty = uint64(stai_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + stai_requested + expected_royalty
    expected_taker_balance = funds - taker_fee - stai_requested - expected_royalty
...
async def test_nft_offer_request_nft(
    ...
    offer_dict = {nft_to_request_asset_id: 1, wallet_maker.id(): -stai_offered}
    ...
    # assert payments and royalties
    expected_royalty = uint64(stai_offered * royalty_basis_pts / 10000)
    expected_maker_balance = funds - maker_fee - stai_offered - expected_royalty
    expected_taker_balance = funds - 2 - taker_fee + stai_offered + expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_did_to_did(
    ...
    expected_royalty = uint64(stai_requested * royalty_basis_pts / 10000)
    expected_maker_balance = funds - 2 - maker_fee + stai_requested + expected_royalty
    expected_taker_balance = funds - 1 - taker_fee - stai_requested - expected_royalty
    await time_out_assert(20, wallet_maker.get_confirmed_balance, expected_maker_balance)
    await time_out_assert(20, wallet_taker.get_confirmed_balance, expected_taker_balance)
...
async def test_nft_offer_sell_nft_for_cat(
    ...
    offer_did_nft_for_stai = {nft_to_offer_asset_id: -1, cat_wallet_maker.id(): cats_requested}
...
async def test_complex_nft_offer(
    ...
    """
    This test is going to create an offer where the maker offers 1 NFT and 1 CAT for 2 NFTs, an STAI and a CAT
    """
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    full_node_server = full_node_api.server
    ...
    maker_stai_royalties_expected = maker_royalty_summary[nft_to_offer_asset_id_maker][0]["amount"]
    ...
    funds_maker = int(funds_maker - FEE + XCH_REQUESTED + maker_stai_royalties_expected)
    funds_taker = int(funds_taker - FEE - XCH_REQUESTED - maker_stai_royalties_expected)
