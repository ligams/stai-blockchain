...
from stai.clvm.spend_sim import CostLogger, sim_and_client
...
async def test_default_transfer_program(cost_logger: CostLogger) -> None:
    async with sim_and_client() as (sim, sim_client):
        ...
        stai_coin = (await sim_client.get_coin_records_by_puzzle_hash(ACS_PH, include_spent_coins=False))[0].coin
        ...
        stai_announcement_spend = make_spend(
            stai_coin,
            ACS,
        ...
        for i in range(1, 3):
            for announcement_combo in itertools.combinations(
                [did_announcement_spend, stai_announcement_spend, cat_announcement_spend], i
            ):
        ...
        full_bundle = cost_logger.add_cost(
            "Ownership only coin (default NFT1 TP) - one child created + update DID + offer CATs + offer STAI",
            SpendBundle(
                [ownership_spend, did_announcement_spend, stai_announcement_spend, cat_announcement_spend], G2Element()
            ),
        )
