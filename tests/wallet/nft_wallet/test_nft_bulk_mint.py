...
async def test_nft_mint_from_did(
    ...
    metadata_list = [
        {
            "program": Program.to(
                [("u", ["https://www.stai.net/img/branding/stai-logo.svg"]), ("h", bytes32.random(seeded_random).hex())]
            ),
    ...
    expected_stai_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_stai_bal)
...
async def test_nft_mint_from_did_rpc(
    ...
    try:
        ...
        target_list = [encode_puzzle_hash((ph_taker), "stai") for x in range(n)]
        royalty_address = encode_puzzle_hash(bytes32.random(seeded_random), "stai")
        royalty_percentage = 300
        fee = 100
        required_amount = n + (fee * n)
        stai_coins = await client.select_coins(
            amount=required_amount, coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG, wallet_id=wallet_maker.id()
        )
        funding_coin = stai_coins[0]
        assert funding_coin.amount >= required_amount
        funding_coin_dict = stai_coins[0].to_json_dict()
        chunk = 5
        ...
        for i in range(0, n, chunk):
            ...
            resp: Dict[str, Any] = await client.nft_mint_bulk(
                ...
                mint_total=n,
                stai_coins=[next_coin.to_json_dict()],
                stai_change_target=funding_coin_dict["puzzle_hash"],
            ...
            spends.append(sb)
            stai_adds = [c for c in sb.additions() if c.puzzle_hash == funding_coin.puzzle_hash]
            assert len(stai_adds) == 1
            next_coin = stai_adds[0]
...
@pytest.mark.anyio
async def test_nft_mint_from_did_multiple_stai(
    self_hostname: str, two_wallet_nodes: Any, trusted: Any, seeded_random: random.Random
) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    ...
    # Grab two coins for testing that we can create a bulk minting with more than 1 stai coin
    stai_coins_1 = await wallet_maker.select_coins(amount=10000, coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG)
    stai_coins_2 = await wallet_maker.select_coins(
        amount=10000,
        coin_selection_config=DEFAULT_COIN_SELECTION_CONFIG.override(excluded_coin_ids=[c.name() for c in stai_coins_1]),
    )
    stai_coins = stai_coins_1.union(stai_coins_2)
    ...
    # confirm that the spend uses the right amount of stai
    expected_stai_bal = funds - fee - mint_total - 1
    await time_out_assert(30, wallet_maker.get_confirmed_balance, expected_stai_bal)
...
@pytest.mark.anyio
async def test_nft_mint_from_stai(
    self_hostname: str, two_wallet_nodes: Any, trusted: Any, seeded_random: random.Random
) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
    ...
    tx_records = await nft_wallet_maker.mint_from_stai(
        metadata_list, DEFAULT_TX_CONFIG, target_list=target_list, mint_number_start=1, mint_total=mint_total, fee=fee
    )
    sb = tx_records[0].spend_bundle
    assert sb is not None
...
@pytest.mark.anyio
async def test_nft_mint_from_stai_rpc(
    two_wallet_nodes_services: SimulatorsAndWalletsServices,
    trusted: Any,
    self_hostname: str,
    seeded_random: random.Random,
...
@pytest.mark.anyio
async def test_nft_mint_from_stai_multiple_stai(
    self_hostname: str, two_wallet_nodes: Any, trusted: Any, seeded_random: random.Random
) -> None:
    full_nodes, wallets, _ = two_wallet_nodes
    full_node_api: FullNodeSimulator = full_nodes[0]
