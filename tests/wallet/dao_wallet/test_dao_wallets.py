...
from stai.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
from stai.util.ints import uint32, uint64, uint128
...
async def test_dao_creation(
    ...
    # Try to create a DAO with more CATs than stai balance
...
async def test_dao_funding(
    ...
    # Create funding spends for stai and cat
    stai_funds = uint64(500000)
    cat_funds = uint64(100000)
    funding_tx = await dao_wallet_0.create_add_funds_to_treasury_spend(stai_funds, DEFAULT_TX_CONFIG)
    ...
    await time_out_assert(20, dao_wallet_0.get_balance_by_asset_type, stai_funds)
    ...
    stai_proposal_inner = generate_simple_proposal_innerpuz(
        treasury_id,
        [recipient_puzzle_hash],
        [proposal_amount_1],
        [None],
    )
    proposal_tx = await dao_wallet_0.generate_new_proposal(stai_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal)
    ...
    await time_out_assert(30, dao_wallet_0.get_balance_by_asset_type, stai_funds - 10000)
    await time_out_assert(30, dao_wallet_0.get_balance_by_asset_type, cat_funds, cat_id)
    await time_out_assert(30, dao_wallet_1.get_balance_by_asset_type, stai_funds - 10000)
...
async def test_dao_proposals(
    ...
    # Create funding spend so the treasury holds some STAI
    stai_funds = uint64(500000)
    funding_tx = await dao_wallet_0.create_add_funds_to_treasury_spend(stai_funds, DEFAULT_TX_CONFIG)
    await wallet_0.wallet_state_manager.add_pending_transaction(funding_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[funding_tx], timeout=60)
    ...
    await time_out_assert(10, dao_wallet_0.get_balance_by_asset_type, stai_funds)
    await time_out_assert(10, dao_wallet_1.get_balance_by_asset_type, stai_funds)
    await time_out_assert(10, dao_wallet_2.get_balance_by_asset_type, stai_funds)

    # Create Proposals

    # Proposal 0: Spend stai to wallet_2.
    recipient_puzzle_hash = await wallet_2.get_new_puzzlehash()
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        stai_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, fee=base_fee
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    ...
    # Proposal 3: Spend stai to wallet_2 (this prop will close as failed)
    ...
    stai_proposal_inner = Program.to(["x"])
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        stai_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, fee=base_fee
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    ...
    await time_out_assert(
        20, dao_wallet_0.get_balance_by_asset_type, stai_funds - proposal_amount_1 + proposal_min_amt - 1
    )
    ...
    await time_out_assert(20, wallet_2.get_confirmed_balance, funds + proposal_amount_1)
    expected_balance = stai_funds - proposal_amount_1 + (3 * proposal_min_amt) - 3 - new_mint_amount
...
async def test_dao_proposal_partial_vote(
    ...
    # Create funding spends for stai
    stai_funds = uint64(500000)
...
async def test_dao_rpc_api(
    ...
    expected_stai = funds - 1 - cat_amt - fee
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_stai)
    ...
    stai_funding_amt = 200000
    stai_tx = await api_0.dao_add_funds_to_treasury(
        dict(
            wallet_id=dao_wallet_0_id,
            amount=stai_funding_amt,
            funding_wallet_id=1,
        )
    )
    txs = [TransactionRecord.from_json_dict(cat_tx["tx"]), TransactionRecord.from_json_dict(stai_tx["tx"])]
    await full_node_api.wait_transaction_records_entered_mempool(records=txs, timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)

    expected_stai -= stai_funding_amt + new_cat_amt
    await time_out_assert(30, wallet_0.get_confirmed_balance, expected_stai)
    ...
    await rpc_state(
        20, api_0.dao_get_treasury_balance, [{"wallet_id": dao_wallet_0_id}], lambda x: x["balances"]["stai"]
    )
    balances = await api_0.dao_get_treasury_balance({"wallet_id": dao_wallet_0_id})
    assert balances["balances"]["stai"] == stai_funding_amt
    ...
    await api_0.cat_spend(
        {
            ...
            "amount": cat_amt // 2,
            "inner_address": encode_puzzle_hash(ph_1, "stai"),
            "fee": fee,
        }
    )
    txs = await wallet_0.wallet_state_manager.tx_store.get_all_unconfirmed()
    ...
    mint_proposal = await api_0.dao_create_proposal(
        {
            ...
            "proposal_type": "mint",
            "amount": uint64(10000),
            "cat_target_address": encode_puzzle_hash(ph_0, "stai"),
            "vote_amount": cat_amt // 2,
            "fee": fee,
        }
    )
...
async def test_dao_rpc_client(
    ...
    try:
        ...
        # fund treasury
        stai_funds = uint64(10000000000)
        funding_tx = await client_0.dao_add_funds_to_treasury(dao_id_0, 1, stai_funds, DEFAULT_TX_CONFIG)
        ...
        await rpc_state(20, client_0.dao_get_treasury_balance, [dao_id_0], lambda x: x["balances"]["stai"], stai_funds)
        ...
        await rpc_state(
            20, client_0.dao_get_treasury_balance, [dao_id_0], lambda x: x["balances"]["stai"], stai_funds - 1000
        )
        ...
        expected_stai = initial_funds - amount_of_cats - new_cat_amt - stai_funds - (2 * fee) - 2 - 9000
        await rpc_state(
            20, client_0.get_wallet_balance, [wallet_0.id()], lambda x: x["confirmed_wallet_balance"], expected_stai
        )
...
async def test_dao_complex_spends(
    ...
    try:
        ...
        stai_funds = uint64(10000000000)
        for _ in range(4):
            funding_tx = await client_0.dao_add_funds_to_treasury(dao_id_0, 1, uint64(stai_funds / 4), DEFAULT_TX_CONFIG)
            cat_funding_tx = await client_0.dao_add_funds_to_treasury(
        ...
        # Test proposal with multiple conditions and stai coins
        additions = [
            {"puzzle_hash": ph_0.hex(), "amount": stai_funds / 4},
            {"puzzle_hash": ph_1.hex(), "amount": stai_funds / 4},
        ]
        proposal = await client_0.dao_create_proposal(
            wallet_id=dao_id_0,
            proposal_type="spend",
        ...
        await rpc_state(20, client_0.dao_get_proposals, [dao_id_0], lambda x: x["proposals"][-1]["closed"], True)
        await rpc_state(20, client_1.dao_get_proposals, [dao_id_1], lambda x: x["proposals"][-1]["closed"], True)
        # check the stai is received and removed from treasury
        await rpc_state(
            20,
            client_1.get_wallet_balance,
            [wallet_1.id()],
            lambda x: x["confirmed_wallet_balance"],
            initial_funds + (stai_funds / 4),
        )
        await rpc_state(
            20,
            client_0.dao_get_treasury_balance,
        ...
        await rpc_state(
            ...
            [dao_id_0],
            lambda x: x["balances"]["stai"] + 1,  # add 1 so result isn't 0
            1,
        )
        await rpc_state(
            20,
...
async def test_dao_concurrency(
    ...
    # Create a proposal for stai spend
    recipient_puzzle_hash = await wallet_2.get_new_puzzlehash()
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(
        stai_proposal_inner, DEFAULT_TX_CONFIG, dao_cat_0_bal, uint64(1000)
    )
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
...
async def test_dao_reorgs(
    ...
    # Check that the funding spend is recognized by both dao wallets
    await time_out_assert(20, dao_wallet_0.get_balance_by_asset_type, stai_funds)
    await time_out_assert(20, dao_wallet_1.get_balance_by_asset_type, stai_funds)
...
async def test_dao_votes(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(stai_proposal_inner, DEFAULT_TX_CONFIG, vote_1, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(stai_proposal_inner, DEFAULT_TX_CONFIG, vote_2, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    vote_5 = uint64(1)
    proposal_tx = await dao_wallet_0.generate_new_proposal(stai_proposal_inner, DEFAULT_TX_CONFIG, vote_5, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(stai_proposal_inner, DEFAULT_TX_CONFIG, fee=base_fee)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node_0, timeout=30)
...
async def test_dao_resync(
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(stai_proposal_inner, DEFAULT_TX_CONFIG, uint64(10))
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallets_synced(wallet_nodes=[wallet_node_0, wallet_node_1], timeout=30)
    ...
    proposal_tx = await dao_wallet_0.generate_new_proposal(stai_proposal_inner, DEFAULT_TX_CONFIG)
    await wallet_0.wallet_state_manager.add_pending_transaction(proposal_tx)
    await full_node_api.wait_transaction_records_entered_mempool(records=[proposal_tx], timeout=60)
    await full_node_api.process_all_wallet_transactions(wallet_0, timeout=60)
    await full_node_api.wait_for_wallets_synced(wallet_nodes=[wallet_node_0, wallet_node_1], timeout=30)
