...
from stai.protocols.shared_protocol import Capability
from stai.protocols.wallet_protocol import RequestAdditions, RespondAdditions, RespondBlockHeaders, SendTransaction
...
from stai.wallet.util.wallet_sync_utils import PeerRequestException
from stai.wallet.wallet_coin_record import WalletCoinRecord
from stai.wallet.wallet_weight_proof_handler import get_wp_fork_point
...
class TestWalletSync:
    ...
    6. Clear all coins from the dust wallet.
       Send to the dust wallet "spam_filter_after_n_txs" coins that are equal in value to "sati_spam_amount".
       Send 1 mojo from the dust wallet. The dust wallet should receive a change coin valued at "sati_spam_amount-1".
    ...
    @pytest.mark.anyio
    @pytest.mark.parametrize(
        "spam_filter_after_n_txs, sati_spam_amount, dust_value",
        [
    ...
    async def test_dusted_wallet(
        ...
        # It's also possible to obtain the current settings for spam_filter_after_n_txs and sati_spam_amount
        # spam_filter_after_n_txs = wallets[0][0].config["spam_filter_after_n_txs"]
        # sati_spam_amount = wallets[0][0].config["sati_spam_amount"]
        # dust_value=1
        ...
        # Current max value for sati_spam_amount is 0.01 XCH.
        ...
        assert sati_spam_amount >= 1
        assert sati_spam_amount <= 10000000000
        assert dust_value >= 1
        ...
        # The dust is only filtered at this point if spam_filter_after_n_txs is 0 and sati_spam_amount is > dust_value.
        if spam_filter_after_n_txs > 0:
            dust_coins = 1
            large_dust_coins = 0
            large_dust_balance = 0
        elif sati_spam_amount <= dust_value:
            dust_coins = 0
            large_dust_coins = 1
        ...
        small_unspent_count = len([r for r in all_unspent if r.coin.amount < sati_spam_amount])
        ...
        log.info(f"sati_spam_amount {sati_spam_amount}")
        ...
        for i in range(large_coins):
            payee_ph = await dust_wallet.get_new_puzzlehash()
            payees.append(Payment(payee_ph, uint64(sati_spam_amount)))
        ...
        large_coin_balance = large_coins * sati_spam_amount

        # Determine whether the filter should have been activated.
        # Make sure the number of coins matches the expected number.
        # At this point, nothing should be getting filtered unless spam_filter_after_n_txs is 0.
        assert dust_coins == spam_filter_after_n_txs
        assert balance == dust_coins * dust_value + large_coins * sati_spam_amount + large_dust_balance
        ...
        if dust_value >= sati_spam_amount:
            large_dust_coins += 1
            large_dust_balance += dust_value
        ...
        for i in range(5):
            ...
            payees.append(Payment(payee_ph, uint64(sati_spam_amount + i)))
            large_coins += 1
            large_coin_balance += sati_spam_amount + i

            payee_ph = await dust_wallet.get_new_puzzlehash()

            # Make sure we are always creating coins with a positive value.
            if sati_spam_amount - dust_value - i > 0:
                payees.append(Payment(payee_ph, uint64(sati_spam_amount - dust_value - i)))
            else:
                payees.append(Payment(payee_ph, uint64(dust_value)))
            # In cases where sati_spam_amount is sufficiently low,
            # the new dust should be considered a large coina and not be filtered.
            if sati_spam_amount <= dust_value:
                large_dust_coins += 1
                large_dust_balance += dust_value
        ...
        # The size of the coin to send the dust wallet is the same as sati_spam_amount
        if sati_spam_amount > 1:
            coin_value = sati_spam_amount
        else:
        ...
        metadata = Program.to(
            [
                ("u", ["https://www.stai.net/img/branding/stai-logo.svg"]),
