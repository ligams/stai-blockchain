...
from stai.types.signing_mode import SigningMode
from stai.types.spend_bundle import SpendBundle
from stai.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from stai.util.config import load_config, lock_and_load_config, save_config
...
from stai.util.streamable import ConversionError, InvalidTypeError
from stai.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from stai.wallet.cat_wallet.cat_utils import CAT_MOD, construct_cat_puzzle
from stai.wallet.cat_wallet.cat_wallet import CATWallet
from stai.wallet.conditions import ConditionValidTimes, Remark
from stai.wallet.derive_keys import master_sk_to_wallet_sk, master_sk_to_wallet_sk_unhardened
...
from stai.wallet.util.query_filter import AmountFilter, HashFilter, TransactionTypeFilter
from stai.wallet.util.transaction_type import TransactionType
from stai.wallet.util.tx_config import DEFAULT_COIN_SELECTION_CONFIG, DEFAULT_TX_CONFIG
...
async def test_send_transaction(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(await wallet_2.get_new_puzzlehash(), "tstai")
...
async def test_spend_clawback_coins(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = await wallet_1_rpc.send_transaction(
        wallet_id=1,
        amount=uint64(500),
        address=encode_puzzle_hash(wallet_2_puzhash, "tstai"),
        tx_config=DEFAULT_TX_CONFIG,
        fee=uint64(0),
    ...
    tx = await wallet_2_rpc.send_transaction(
        wallet_id=1,
        amount=uint64(500),
        address=encode_puzzle_hash(wallet_1_puzhash, "tstai"),
        tx_config=DEFAULT_TX_CONFIG,
        fee=uint64(0),
        puzzle_decorator_override=[{"decorator": "CLAWBACK", "clawback_timelock": 5}],
    )
...
async def test_get_transactions(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    await client.send_transaction(
        1, uint64(1), encode_puzzle_hash(puzhash, "tstai"), DEFAULT_TX_CONFIG
    )  # Create a pending tx
    ...
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    await client.send_transaction(1, uint64(1), encode_puzzle_hash(ph_by_addr, "tstai"), DEFAULT_TX_CONFIG)
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
    tx_for_address = await client.get_transactions(1, to_address=encode_puzzle_hash(ph_by_addr, "tstai"))
...
async def test_offer_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    # Create an offer of 5 stai for one CAT
...
async def test_get_coin_records_by_names(wallet_rpc_environment: WalletRpcTestEnvironment) -> None:
    ...
    address = encode_puzzle_hash(await env.wallet_1.wallet.get_new_puzzlehash(), "tstai")
    await full_node_api.wait_for_wallet_synced(wallet_node=wallet_node, timeout=20)
...
async def test_nft_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    res = await wallet_1_rpc.mint_nft(
        ...
        ["https://www.stai.net/img/branding/stai-logo.svg"],
        DEFAULT_TX_CONFIG,
    )
    assert res["success"]
...
async def test_key_and_address_endpoints(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    addr = encode_puzzle_hash(ph, "tstai")
    tx_amount = uint64(15600000)
...
# Example 1:
# $ stai keys generate
# $ stai keys sign -d 'hello world' -t 'm/12381/8444/1/1'
#
# Example 2:
# $ stai wallet get_address
# stai1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
# $ stai wallet sign_message -m $(echo -n 'hello world' | xxd -p)
# -a stai1vk0dj7cx7d638h80mcuw70xqlnr56pmuhzajemn5ym02vhl3mzyqrrd4wp
#
@pytest.mark.parametrize(
    ...
    [
        # Valid signatures
        (
            # stai keys sign -d "Let's eat, Grandma" -t "m/12381/8444/1/1"
            {
        ...
        (
            # stai wallet sign_message -m $(echo -n 'Happy happy joy joy' | xxd -p)
            # -a stai1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
        ...
        (
            ...
            # -a stai1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "stai1e2pcue5q7t4sg8gygz3aht369sk78rzzs92zx65ktn9a9qurw35saajvkh",
            },
            {"isValid": True},
        ),
        (
            {
                ...
                "address": "stai1hh9phcc8tt703dla70qthlhrxswy88va04zvc7vd8cx2v6a5ywyst8mgul",
            },
            {"isValid": True},
        ),
        # Negative tests
        ...
        (
            # Valid signature but address doesn't match pubkey
            {
                ...
                "signing_mode": SigningMode.CHIP_0002.value,
                "address": "stai1d0rekc2javy5gpruzmcnk4e4qq834jzlvxt5tcgl2ylt49t26gdsjen7t0",
            },
...
async def test_cat_spend_run_tail(wallet_rpc_environment: WalletRpcTestEnvironment):
    ...
    tx = await client.cat_spend(
        ...
        amount=uint64(0),
        tx_config=DEFAULT_TX_CONFIG,
        inner_address=encode_puzzle_hash(our_ph, "tstai"),
