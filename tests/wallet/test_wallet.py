...
from stai.simulator.full_node_simulator import FullNodeSimulator, wait_for_coins_in_wallet
...
from stai.types.coin_spend import compute_additions
from stai.types.peer_info import PeerInfo
from stai.types.signing_mode import CHIP_0002_SIGN_MESSAGE_PREFIX
from stai.types.spend_bundle import estimate_fees
...
from stai.wallet.util.query_filter import TransactionTypeFilter
...
from stai.wallet.util.wallet_types import CoinType
from stai.wallet.wallet_node import WalletNode, get_wallet_db_path
...
class TestWalletSimulator:
    ...
    async def test_wallet_make_transaction(
        self,
        two_wallet_nodes: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 5
    ...
    async def test_wallet_clawback_clawback(
        ...
        stai_tx = resp["transactions"][0]
        assert list(stai_tx["memos"].values())[0] == b"Test".hex()
    ...
    async def test_wallet_send_to_three_peers(
        self,
        three_sim_two_wallets: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
        num_blocks = 10
    ...
    async def test_address_sliding_window(
        self,
        wallet_node_100_pk: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]], BlockTools],
        trusted: bool,
        self_hostname: str,
    ) -> None:
    ...
    async def test_sign_message(
        ...
        # Test general string
        message = "Hello World"
        response = await api_0.sign_message_by_address({"address": encode_puzzle_hash(ph, "stai"), "message": message})
        ...
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "stai"), "message": message, "is_hex": True}
        )
        ...
        message = "0123456789ABCDEF"
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "stai"), "message": message, "is_hex": "true", "safe_mode": "true"}
        )
        puzzle = Program.to((CHIP_0002_SIGN_MESSAGE_PREFIX, bytes.fromhex(message)))
        ...
        message = "Hello World"
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "stai"), "message": message, "is_hex": False, "safe_mode": False}
        )
        ...
        message = "0123456789ABCDEF"
        response = await api_0.sign_message_by_address(
            {"address": encode_puzzle_hash(ph, "stai"), "message": message, "is_hex": True, "safe_mode": False}
        )
...
def test_get_wallet_db_path_v2_r1() -> None:
    root_path: Path = Path("/x/y/z/.stai/mainnet").resolve()
    config: Dict[str, Any] = {
...
def test_get_wallet_db_path_testnet() -> None:
    root_path: Path = Path("/x/y/z/.stai/testnet").resolve()
    config: Dict[str, Any] = {
        "database_path": "wallet/db/blockchain_wallet_v2_CHALLENGE_KEY.sqlite",
