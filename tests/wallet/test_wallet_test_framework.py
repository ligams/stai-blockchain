...
async def test_basic_functionality(wallet_environments: WalletTestFramework) -> None:
    ...
    assert await env_0.stai_wallet.get_confirmed_balance() == 2_000_000_000_000
    assert await env_1.stai_wallet.get_confirmed_balance() == 0
...
async def test_balance_checking(
    ...
    await wallet_environments.full_node.farm_blocks_to_wallet(count=1, wallet=env_0.stai_wallet)
    ...
    await CATWallet.get_or_create_wallet_for_cat(env_0.wallet_state_manager, env_0.stai_wallet, "00" * 32)
    ...
    with pytest.raises(BalanceCheckingError, match="not in balance response"):
        await env_0.check_balances(additional_balance_info={"stai": {"something not there": 0}})
