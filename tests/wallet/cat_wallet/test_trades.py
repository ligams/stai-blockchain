...
from stai.types.blockchain_format.program import INFINITE_COST, Program
...
async def test_cat_trades(
    ...
    wallet_maker = env_maker.stai_wallet
    wallet_taker = env_taker.stai_wallet
    ...
    stai_for_multiple_cat: OfferSummary = {
        wallet_maker.id(): -7,
        cat_wallet_maker.id(): 8,
        new_cat_wallet_maker.id(): 9,
    }
    multiple_cat_for_stai: OfferSummary = {
        wallet_maker.id(): 10,
        cat_wallet_maker.id(): -11,
        new_cat_wallet_maker.id(): -12,
    }
    stai_and_cat_for_cat: OfferSummary = {
        wallet_maker.id(): -13,
    ...
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        stai_for_cat, wallet_environments.tx_config, fee=uint64(1)
    )
        ...
        [
            WalletStateTransition(
                ...
                post_block_balance_updates={
                    "stai": {
                        "pending_coin_removal_count": -1,
                        "confirmed_wallet_balance": -2,  # One for offered STAI, one for fee
                        "unconfirmed_wallet_balance": -2,  # One for offered STAI, one for fee
                        ">#spendable_balance": 0,
                        ">#max_send_amount": 0,
                    },
                    "new cat": {
            ...
            WalletStateTransition(
                pre_block_balance_updates={
                    "stai": {
                        ...
                        "<=#spendable_balance": -2,
                        "<=#max_send_amount": -2,
                        # Unconfirmed balance doesn't change because receiveing 1 STAI and spending 1 in fee
                        "unconfirmed_wallet_balance": 0,
                    },
                    "new cat": {
                        "unconfirmed_wallet_balance": -2,
                ...
                post_block_balance_updates={
                    "stai": {
                        ...
                        ">#spendable_balance": 0,
                        ">#max_send_amount": 0,
                        # Confirmed balance doesn't change because receiveing 1 STAI and spending 1 in fee
                        "confirmed_wallet_balance": 0,
                    },
                    "new cat": {
                        "confirmed_wallet_balance": -2,
    ...
    # cat_for_stai
    success, trade_make, error = await trade_manager_maker.create_offer_for_ids(
        cat_for_stai, wallet_environments.tx_config
    )
    assert error is None
    assert success is True
    assert trade_make is not None
    ...
    stai_tx: TransactionRecord = next(tx for tx in tx_records if tx.wallet_id == 1)
    assert stai_tx.amount == 3
    assert stai_tx.fee_amount == 1
...
class TestCATTrades:
    ...
    async def test_trade_cancellation(self, wallets_prefarm):
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, stai_to_cat_amount)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, stai_to_cat_amount)
        maker_funds -= stai_to_cat_amount
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(cat_for_stai, DEFAULT_TX_CONFIG)
        assert error is None
        assert success is True
        assert trade_make is not None
        ...
        success, trade_make, error = await trade_manager_maker.create_offer_for_ids(stai_for_cat, DEFAULT_TX_CONFIG)
        assert error is None
        assert success is True
        assert trade_make is not None
    ...
    async def test_aggregated_trade_state(self, wallets_prefarm):
        ...
        success, trade_make_1, error = await trade_manager_maker.create_offer_for_ids(stai_for_cat, DEFAULT_TX_CONFIG)
        ...
        success, trade_make_2, error = await trade_manager_maker.create_offer_for_ids(cat_for_stai, DEFAULT_TX_CONFIG)
        ...
        await time_out_assert(15, cat_wallet_maker.get_confirmed_balance, stai_to_cat_amount - 1)
        await time_out_assert(15, cat_wallet_maker.get_unconfirmed_balance, stai_to_cat_amount - 1)
