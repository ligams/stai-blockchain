...
from stai.clvm.spend_sim import sim_and_client
...
from stai.wallet.trading.offer import OFFER_MOD, NotarizedPayment, Offer
...
# This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/STAI coins
...
class TestOfferLifecycle:
    ...
    async def test_complex_offer(self, cost_logger):
        async with sim_and_client() as (sim, sim_client):
            ...
            stai_coins: List[Coin] = all_coins[None]
            ...
            # Create an STAI Offer for RED
            stai_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            stai_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                stai_requested_payments, stai_coins
            )
            stai_announcements: List[Announcement] = Offer.calculate_announcements(stai_requested_payments, driver_dict)
            stai_secured_bundle: SpendBundle = generate_secure_bundle(stai_coins, stai_announcements, 1000)
            stai_offer = Offer(stai_requested_payments, stai_secured_bundle, driver_dict)
            assert not stai_offer.is_valid()

            # Create a RED Offer for STAI
            ...
            new_offer = Offer.aggregate([stai_offer, red_offer, red_offer_2])
            ...
            # Create yet another offer of BLUE for STAI and RED
            ...
            assert new_offer.summary() == (
                ...
                {"stai": 900, str_to_tail_hash("red").hex(): 350},
            ...
            # Test preventing TAIL from running during estaiange
