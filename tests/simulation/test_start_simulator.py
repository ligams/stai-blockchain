...
from stai.simulator.simulator_test_tools import get_full_stai_simulator, get_puzzle_hash_from_key
...
class TestStartSimulator:
    ...
    async def get_stai_simulator(
        self, tmp_path: Path, empty_keyring: Keychain
    ) -> AsyncGenerator[Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain], None]:
        async for simulator_args in get_full_stai_simulator(stai_root=tmp_path, keychain=empty_keyring):
            yield simulator_args

    @pytest.mark.anyio
    async def test_start_simulator(
        self, get_stai_simulator: Tuple[FullNodeSimulator, Path, Dict[str, Any], str, int, Keychain]
    ) -> None:
        simulator, root_path, config, mnemonic, fingerprint, keychain = get_stai_simulator
