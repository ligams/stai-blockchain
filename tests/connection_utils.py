...
async def disconnect_all(server: StaiServer) -> None:
...
async def add_dummy_connection_wsc(
    server: StaiServer, self_hostname: str, dummy_port: int, type: NodeType = NodeType.FULL_NODE
) -> Tuple[WSStaiConnection, bytes32]:
    ...
    if type in authenticated_client_types:
        ...
        stai_ca_crt_path, stai_ca_key_path = stai_ssl_ca_paths(server.root_path, config)
        ca_crt_path = stai_ca_crt_path
        ca_key_path = stai_ca_key_path
