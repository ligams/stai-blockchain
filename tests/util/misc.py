...
from stai.wallet.util.compute_hints import HintedCoin
...
class _AssertRuntime:
    ...
    Produces output of the following form.
        ...
        /home/altendky/repos/stai-blockchain/tests/core/full_node/test_performance.py:187
    ...
    def __exit__(
        ...
        if self.record_property is not None:
            ...
            relative_path_str = (
                pathlib.Path(results.entry_file).relative_to(pathlib.Path(stai.__file__).parent.parent).as_posix()
            )
...
@contextlib.contextmanager
def closing_stai_root_popen(stai_root: StaiRoot, args: List[str]) -> Iterator[subprocess.Popen[Any]]:
    environment = {**os.environ, "STAI_ROOT": os.fspath(stai_root.path)}
