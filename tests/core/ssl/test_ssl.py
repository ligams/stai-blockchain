...
from stai.protocols.shared_protocol import capabilities, protocol_version
...
async def establish_connection(server: StaiServer, self_hostname: str, ssl_context) -> None:
...
class TestSSL:
    ...
    async def test_farmer(self, farmer_one_harvester, self_hostname):
        ...
        stai_ca_crt_path, stai_ca_key_path = stai_ssl_ca_paths(bt.root_path, bt.config)
        ...
        generate_ca_signed_cert(stai_ca_crt_path.read_bytes(), stai_ca_key_path.read_bytes(), pub_crt, pub_key)
        ssl_context = ssl_context_for_client(stai_ca_crt_path, stai_ca_key_path, pub_crt, pub_key)
    ...
    async def test_full_node(self, simulator_and_wallet, self_hostname):
        ...
        generate_ca_signed_cert(
            stai_ca_crt_path.read_bytes(),
            stai_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(stai_ca_crt_path, stai_ca_key_path, pub_crt, pub_key)
    ...
    async def test_introducer(self, introducer_service, self_hostname):
        ...
        stai_ca_crt_path, stai_ca_key_path = stai_ssl_ca_paths(introducer_service.root_path, introducer_service.config)
