...
from stai.daemon.client import connect_to_daemon
from stai.daemon.keychain_server import (
    DeleteLabelRequest,
    GetKeyRequest,
    GetKeyResponse,
    GetKeysResponse,
...
from stai.daemon.server import WebSocketServer, plotter_log_path, service_plotter
from stai.plotters.plotters import call_plotters
from stai.simulator.block_tools import BlockTools
from stai.simulator.keyring import TempKeyring
from stai.simulator.setup_services import setup_full_node
from stai.util.config import load_config
from stai.util.json_util import dict_to_json_str
from stai.util.keychain import Keychain, KeyData, supports_os_passphrase_storage
from stai.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE, KeyringWrapper
from stai.util.ws_message import create_payload, create_payload_dict
from stai.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk
...
@dataclass
class StaiPlottersBladebitArgsCase:
    case_id: str
    plot_type: str
    count: int = 1
    threads: int = 0
    pool_contract: str = "tstai1xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
    compress: int = 1
    device: int = 0
...
def mock_daemon_with_services():
    ...
    return Daemon(
        services={
            ...
            "stai_plotter": [Service(True), Service(True)],
        },
        connections={},
        net_config={},
    )
    ...
    async with aiohttp.ClientSession() as client:
        async with client.ws_connect(
            ...
            async with setup_full_node(
                ...
                await time_out_assert_not_none(30, ws_server.connections.get, "stai_full_node")

                payload = create_payload("get_blockchain_state", {}, service_name, "stai_full_node")
                await ws.send_str(payload)

                response = await ws.receive()
                message = assert_response_success_only(response)
                assert message["command"] == "get_blockchain_state"
                assert message["origin"] == "stai_full_node"
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "stai1zze67l3jgxuvyaxhjhu7326sezxxve7lgzvq0497ddggzhff7c9s2pdcwh",
                        "hd_path": "m/12381/8444/2/0",
                    },
                ],
                test_key_data_2.fingerprint: [
                    {
                        "address": "stai1fra5h0qnsezrxenjyslyxx7y4l268gq52m0rgenh58vn8f577uzswzvk4v",
                        "hd_path": "m/12381/8444/2/0",
                    }
                ],
            },
    ...
    WalletAddressCase(
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "stai16jqcaguq27z8xvpu89j7eaqfzn6k89hdrrlm0rffku85n8n7m7sqqmmahh",
                        "hd_path": "m/12381/8444/2/1",
                    },
                    {
                        "address": "stai1955vj0gx5tqe7v5tceajn2p4z4pup8d4g2exs0cz4xjqses8ru6qu8zp3y",
    ...
    WalletAddressCase(
        ...
        response={
            ...
            "wallet_addresses": {
                test_key_data.fingerprint: [
                    {
                        "address": "stai1k996a7h3agygjhqtrf0ycpa7wfd6k5ye2plkf54ukcmdj44gkqkq880l7n",
...
@datacases(
    ...
    RouteCase(
        route="register_service",
        description="stai_plotter",
        request={
            "service": "stai_plotter",
        },
        response={"success": True, "service": "stai_plotter", "queue": []},
    ),
    RouteCase(
    ...
    RouteCase(
        ...
        response={
            ...
            "plotters": {
                ...
                "chiapos": {"display_name": "Stai Proof of Space", "installed": True, "version": chiapos_version},
...
async def test_plotter_options(
    ...
    # register for stai_plotter events
    service_name = "stai_plotter"
    data = {"service": service_name}
    payload = create_payload("register_service", data, "stai_plotter", "daemon")
    await ws.send_str(payload)
    response = await ws.receive()
...
@datacases(
    StaiPlottersBladebitArgsCase(case_id="1", plot_type="cudaplot"),
    StaiPlottersBladebitArgsCase(case_id="2", plot_type="cudaplot", hybrid_disk_mode=16),
    StaiPlottersBladebitArgsCase(case_id="3", plot_type="cudaplot", hybrid_disk_mode=128),
)
def test_run_plotter_bladebit(
    mocker: MockerFixture,
    mock_daemon_with_config_and_keys,
    bt: BlockTools,
    case: StaiPlottersBladebitArgsCase,
) -> None:
    ...
    mocker.patch("stai.plotters.bladebit.get_bladebit_version", side_effect=get_bladebit_version)
    mock_run_plotter = mocker.patch("stai.plotters.bladebit.run_plotter")
