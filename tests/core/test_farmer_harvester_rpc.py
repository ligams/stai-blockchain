...
from stai.protocols import farmer_protocol
from stai.protocols.harvester_protocol import Plot
from stai.rpc.farmer_rpc_api import (
    FilterItem,
...
from stai.simulator.block_tools import get_plot_dir
...
from stai.util.misc import get_list_or_len
...
async def test_farmer_reward_target_endpoints(harvester_farmer_environment: HarvesterFarmerEnvironment) -> None:
    ...
    await farmer_rpc_client.set_reward_targets(encode_puzzle_hash(new_ph, "stai"), encode_puzzle_hash(new_ph_2, "stai"))
    ...
    await farmer_rpc_client.set_reward_targets(
        encode_puzzle_hash(observer_farmer, "stai"), encode_puzzle_hash(observer_pool, "stai")
    )
    ...
    assert config["farmer"]["stai_target_address"] == encode_puzzle_hash(observer_farmer, "stai")
    assert config["pool"]["stai_target_address"] == encode_puzzle_hash(observer_pool, "stai")

    new_ph_2_encoded = encode_puzzle_hash(new_ph_2, "stai")
