...
from stai.server.stai_policy import StaiSelectorEventLoop, PausableServer, _stai_create_server, set_stai_policy


def test_base_event_loop_has_methods() -> None:
    """
    `StaiPolicy` overrides `create_server` to create and return the custom `PausableServer`
    ...
    Moreover, this checks our internal method `_stai_create_server` doesn't change signature and that our
    ...
    selector_event_loop = StaiSelectorEventLoop()
    try:
        base_selector_event_loop = super(StaiSelectorEventLoop, selector_event_loop)
        ...
        assert inspect.isfunction(_stai_create_server)
        ...
        assert str(inspect.signature(_stai_create_server)) == expected_signature
        ...
        def in_thread() -> None:
            ...
            set_stai_policy(connection_limit=0)
            asyncio.run(main())
