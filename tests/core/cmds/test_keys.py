...
from stai.cmds.keys import delete_all_cmd, generate_and_print_cmd, sign_cmd, verify_cmd
from stai.util.config import load_config
from stai.util.default_root import DEFAULT_KEYS_ROOT_PATH
from stai.util.keychain import Keychain, KeyData, generate_mnemonic
from stai.util.keyring_wrapper import KeyringWrapper
...
class TestKeysCommands:
    def test_generate_with_new_config(self, tmp_path, empty_keyring):
        ...
        the correct stai_target_address entries.
        """
        ...
        # Verify that the config has the correct stai_target_address entries
        address_matches = re.findall(r"stai1[^\n]+", result.output)
        assert len(address_matches) > 1
        address = address_matches[0]

        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["stai_target_address"] == address
        assert config["pool"]["stai_target_address"] == address
    ...
    def test_generate_with_existing_config(self, tmp_path, empty_keyring):
        ...
        the original stai_target_address entries.
        """
        ...
        assert existing_config["farmer"]["stai_target_address"] == address
        assert existing_config["pool"]["stai_target_address"] == address
        ...
        # Verify that the config's stai_target_address entries have not changed
        config: Dict = load_config(tmp_path, "config.yaml")
        assert config["farmer"]["stai_target_address"] == existing_config["farmer"]["stai_target_address"]
        assert config["pool"]["stai_target_address"] == existing_config["pool"]["stai_target_address"]
    ...
    def test_show_labels(self, empty_keyring, tmp_path):
        ...
        result = runner.invoke(cli, [*base_params, *cmd_params], catch_exceptions=False)
        assert result.output == "No keys are present in the keychain. Generate them with 'stai keys generate'\n"
    ...
    def test_show(self, keyring_with_one_key, tmp_path):
        """
        Test that the `stai keys show` command shows the correct key.
        """
    ...
    def test_show_fingerprint(self, keyring_with_one_key, tmp_path):
        """
        Test that the `stai keys show --fingerprint` command shows the correct key.
        """
    ...
    def test_show_json(self, keyring_with_one_key, tmp_path):
        """
        Test that the `stai keys show --json` command shows the correct key.
        """
    ...
    def test_show_mnemonic(self, keyring_with_one_key, tmp_path):
        """
        Test that the `stai keys show --show-mnemonic-seed` command shows the key's mnemonic seed.
        """
    ...
    def test_show_mnemonic_json(self, keyring_with_one_key, tmp_path):
        """
        Test that the `stai keys show --show-mnemonic-seed --json` command shows the key's mnemonic seed.
        """
    ...
    def test_generate_and_print(self):
        """
        Test the `stai keys generate_and_print` command.
        """
    ...
    def test_sign(self, keyring_with_one_key):
        """
        Test the `stai keys sign` command.
        """
    ...
    def test_sign_non_observer(self, keyring_with_one_key):
        """
        Test the `stai keys sign` command with a non-observer key.
        """
    ...
    def test_verify(self):
        """
        Test the `stai keys verify` command.
        """
    ...
    def test_derive_search(self, tmp_path, keyring_with_one_key):
        """
        Test the `stai keys derive search` command, searching a public and private key
        """
    ...
    def test_derive_search_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `stai keys derive search` command, searching for a wallet address
        """
        ...
        result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "stai1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd",
            ],
            catch_exceptions=False,
        )
        ...
        assert (
            result.output.find(
                ...
                "stai1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs8taffd (HD path: m/12381/8444/2/30)"
            )
            != -1
        )
    ...
    def test_derive_search_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `stai keys derive search` command, searching for a testnet wallet address
        """
        ...
        result = runner.invoke(
            ...
            [
                ...
                "--search-type",
                "address",
                "tstai1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs2v6lg7",
                "--prefix",
                "tstai",
            ],
            catch_exceptions=False,
        ...
        assert (
            result.output.find(
                "Found wallet address: "
                "tstai1mnr0ygu7lvmk3nfgzmncfk39fwu0dv933yrcv97nd6pmrt7fzmhs2v6lg7 (HD path: m/12381/8444/2/30)"
            )
            != -1
        )
    ...
    def test_derive_search_failure(self, tmp_path, keyring_with_one_key):
        """
        Test the `stai keys derive search` command with a failing search.
        """
    ...
    def test_derive_search_hd_path(self, tmp_path, empty_keyring, mnemonic_seed_file):
        """
        Test the `stai keys derive search` command, searching under a provided HD path.
        """
    ...
    def test_derive_wallet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `stai keys derive wallet-address` command, generating a couple of wallet addresses.
        """
        ...
        assert (
            result.output.find(
                ...
                "stai1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfs6wrfdq"
            )
            != -1
        )
        assert (
            result.output.find(
                ...
                "stai1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8sumvfmt"
            )
            != -1
        )
    ...
    def test_derive_wallet_testnet_address(self, tmp_path, keyring_with_one_key):
        """
        Test the `stai keys derive wallet-address` command, generating a couple of testnet wallet addresses.
        """
        ...
        assert (
            result.output.find(
                "Wallet address 50 (m/12381n/8444n/2n/50n): "
                "tstai1jp2u7an0mn9hdlw2x05nmje49gwgzmqyvh0qmh6008yksetuvkfshfylvn"
            )
            != -1
        )
        assert (
            result.output.find(
                "Wallet address 51 (m/12381n/8444n/2n/51n): "
                "tstai1006n6l3x5e8exar8mlj004znjl5pq0tq73h76kz0yergswnjzn8s3utl6c"
            )
            != -1
        )
    ...
    def test_derive_child_keys(self, tmp_path, keyring_with_one_key):
        """
        Test the `stai keys derive child-keys` command, generating a couple of derived keys.
        """
