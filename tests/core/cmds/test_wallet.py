...
from stai.cmds.wallet_funcs import print_offer_summary
...
@pytest.mark.anyio
async def test_print_offer_summary_stai(capsys: Any) -> None:
    summary_dict = {"stai": 1_000_000_000_000}

    await print_offer_summary(cat_name_resolver, summary_dict)

    captured = capsys.readouterr()

    assert "STAI (Wallet ID: 1): 1.0 (1000000000000 mojos)" in captured.out
...
@pytest.mark.anyio
async def test_print_offer_summary_stai_and_cats(capsys: Any) -> None:
    summary_dict = {
        "stai": 2_500_000_000_000,
    ...
    assert "STAI (Wallet ID: 1): 2.5 (2500000000000 mojos)" in captured.out
...
@pytest.mark.anyio
async def test_print_offer_summary_stai_and_cats_with_zero_values(capsys: Any) -> None:
    summary_dict = {
        "stai": 0,
    ...
    assert "STAI (Wallet ID: 1): 0.0 (0 mojos)" in captured.out
...
@pytest.mark.anyio
async def test_print_offer_summary_stai_with_one_mojo(capsys: Any) -> None:
    summary_dict = {"stai": 1}
    ...
    assert "STAI (Wallet ID: 1): 1e-12 (1 mojo)" in captured.out
