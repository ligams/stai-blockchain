...
from stai.cmds.beta_funcs import default_beta_root_path
from stai.cmds.stai import cli
from stai.util.beta_metrics import metrics_log_interval_default, metrics_log_interval_max, metrics_log_interval_min
...
def generate_example_submission_data(beta_root_path: Path, versions: int, logs: int) -> None:
    for version in range(versions):
        ...
        version_path.mkdir()
        stai_blockchain_logs = version_path / "stai-blockchain"
        plotting_logs = version_path / "plotting"
        stai_blockchain_logs.mkdir()
        plotting_logs.mkdir()
        for i in range(logs):
            with open(stai_blockchain_logs / f"beta_{i}.log", "w"):
                pass
            with open(stai_blockchain_logs / f"beta_{i + 10}.gz", "w"):
                pass
...
def test_configure_no_beta_config(root_path_populated_with_config: Path) -> None:
    ...
    assert "beta test mode is not enabled, enable it first with `stai beta enable`" in result.output
...
def test_prepare_submission(
    ...
    if exit_code == 0:
        ...
        with zipfile.ZipFile(submission_file) as zip_file:
            ...
            for version in range(versions):
                stai_blockchain_logs = Path("stai-blockchain")
                plotting_logs = Path("plotting")
                for i in range(logs):
                    assert stai_blockchain_logs / f"beta_{i}.log" in all_files
                    assert stai_blockchain_logs / f"beta_{i + 10}.gz" in all_files
