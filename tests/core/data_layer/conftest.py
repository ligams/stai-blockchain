...
from stai.data_layer.data_layer_util import NodeType, Status
...
@pytest.fixture(name="stai_daemon", scope="function")
def stai_daemon_fixture(stai_root: StaiRoot) -> Iterator[None]:
    with closing_stai_root_popen(stai_root=stai_root, args=[sys.executable, "-m", "stai.daemon.server"]):
...
@pytest.fixture(name="stai_data", scope="function")
def stai_data_fixture(stai_root: StaiRoot, stai_daemon: None, scripts_path: pathlib.Path) -> Iterator[None]:
    with closing_stai_root_popen(stai_root=stai_root, args=[os.fspath(scripts_path.joinpath("stai_data_layer"))]):
        # TODO: this is not pretty as a hard coded time
        # let it settle
        time.sleep(5)
        yield
