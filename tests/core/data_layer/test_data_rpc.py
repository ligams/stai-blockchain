...
from stai.cmds.data_funcs import clear_pending_roots, wallet_log_in_cmd
...
from stai.data_layer.data_layer_errors import OfferIntegrityError
from stai.data_layer.data_layer_util import OfferStore, Status, StoreProofs
from stai.data_layer.data_layer_wallet import DataLayerWallet, verify_offer
from stai.data_layer.download_data import get_delta_filename, get_full_tree_filename
...
from stai.server.start_data_layer import create_data_layer_service
...
from stai.types.aliases import DataLayerService, WalletService
...
from stai.util.config import save_config
...
from stai.wallet.trading.offer import Offer as TradingOffer
...
async def run_cli_cmd(*args: str, root_path: Path) -> asyncio.subprocess.Process:
    process = await asyncio.create_subprocess_exec(
        ...
        "stai",
        *args,
        env={**os.environ, "STAI_ROOT": os.fspath(root_path)},
...
async def test_clear_pending_roots(
    ...
    async with init_data_layer_service(wallet_rpc_port=wallet_rpc_port, bt=bt, db_path=tmp_path) as data_layer_service:
        ...
        if layer == InterfaceLayer.direct:
            ...
            process = await asyncio.create_subprocess_exec(
                *args,
                env={**os.environ, "STAI_ROOT": str(bt.root_path)},
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
            )
            await process.wait()
