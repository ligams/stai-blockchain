...
from stai.data_layer.data_layer_util import PluginRemote
...
@pytest.mark.anyio
async def test_sql_logs(enable: bool, config: Dict[str, Any], tmp_stai_root: Path) -> None:
    config["data_layer"]["log_sqlite_cmds"] = enable

    log_path = tmp_stai_root.joinpath("log", "data_sql.log")

    data_layer = DataLayer.create(
        config=config["data_layer"],
        root_path=tmp_stai_root,
