...
class TestBlockchainTransactions:
    ...
    async def test_basic_blockchain_tx(
        self, two_nodes: Tuple[FullNodeAPI, FullNodeAPI, StaiServer, StaiServer, BlockTools]
    ) -> None:
        num_blocks = 10
        wallet_a = WALLET_A
    ...
    async def test_validate_blockchain_spend_reorg_coin(
        self, two_nodes: Tuple[FullNodeAPI, FullNodeAPI, StaiServer, StaiServer, BlockTools], softfork_height: uint32
    ) -> None:
        num_blocks = 10
        wallet_a = WALLET_A
        coinbase_puzzlehash = WALLET_A_PUZZLE_HASHES[0]
