...
from stai.clvm.spend_sim import CostLogger
...
from stai.seeder.dns_server import DNSServer
...
from stai.simulator.setup_services import setup_crawler, setup_daemon, setup_introducer, setup_seeder, setup_timelord
from stai.simulator.wallet_tools import WalletTool
from stai.timelord.timelord import Timelord
from stai.timelord.timelord_api import TimelordAPI

# Set spawn after stdlib imports, but before other imports
from stai.types.aliases import (
    CrawlerService,
    DataLayerService,
    FarmerService,
    FullNodeService,
...
from stai.util.config import create_default_stai_config, lock_and_load_config
from stai.util.db_wrapper import generate_in_memory_db_uri
from stai.util.ints import uint16, uint32, uint64
from stai.util.keychain import Keychain
from stai.util.task_timing import main as task_instrumentation_main
from stai.util.task_timing import start_task_instrumentation, stop_task_instrumentation
...
from stai.simulator.block_tools import BlockTools, create_block_tools_async, test_constants
...
# NOTE:
#       Instantiating the bt fixture results in an attempt to create the stai root directory
...
def pytest_collection_modifyitems(session, config: pytest.Config, items: List[pytest.Function]):
    ...
    if len(all_error_lines) > 0:
        all_error_lines.insert(0, "custom stai collection rules failed")
...
) -> AsyncIterator[Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, StaiServer]], BlockTools]]:
...
async def two_wallet_nodes_custom_spam_filtering(
    spam_filter_after_n_txs, sati_spam_amount, blockchain_constants: ConsensusConstants
):
    async with setup_simulators_and_wallets(
        1, 2, blockchain_constants, spam_filter_after_n_txs, sati_spam_amount
    ) as new:
        yield make_old_setup_simulators_and_wallets(new=new)
...
async def one_node_one_block(
    blockchain_constants: ConsensusConstants,
) -> AsyncIterator[Tuple[Union[FullNodeAPI, FullNodeSimulator], StaiServer, BlockTools]]:
...
@pytest.fixture(scope="function")
def empty_keyring():
    with TempKeyring(user="user-stai-1.8", service="stai-user-stai-1.8") as keychain:
        yield keychain
        KeyringWrapper.cleanup_shared_instance()
...
@pytest.fixture(scope="function")
def tmp_stai_root(tmp_path):
    """
    Create a temp directory and populate it with an empty stai_root directory.
    """
    path: Path = tmp_path / "stai_root"
    path.mkdir(parents=True, exist_ok=True)
    return path


@pytest.fixture(scope="function")
def root_path_populated_with_config(tmp_stai_root) -> Path:
    """
    Create a temp stai_root directory and populate it with a default config.yaml.
    Returns the stai_root path.
    """
    root_path: Path = tmp_stai_root
    create_default_stai_config(root_path)
    return root_path
...
@pytest.fixture(name="stai_root", scope="function")
def stai_root_fixture(tmp_path: Path, scripts_path: Path) -> StaiRoot:
    root = StaiRoot(path=tmp_path.joinpath("stai_root"), scripts_path=scripts_path)
    root.run(args=["init"])
